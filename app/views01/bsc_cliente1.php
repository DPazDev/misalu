  ...
  $numfilas=num_filas($resuCedula);
    ...
    $numfTitular=num_filas($resuTitular);
       if ($numfTitular>0){
	     ...
	     //ver a que ente pertenece y ver su estatus
	       $queryEnte=("select entes.nombre, estados_clientes.estado_cliente,titulares.id_titular where titulares.id_ente=entes.id_ente and titulares.id_titular=estados_t_b.id_titular and estados_t_b.id_estado_cliente=estados_clientes.id_estado_cliente and titulares.id_cliente=clientes.id_cliente and clientes.cedula='$ceduclien' and estados_t_b.id_beneficiario=0 order by estado_cliente;");
	       $resuEnte=ejecutar($queryEnte);
              //busco a ver si tiene beneficiarios y su entes y su estatus
	      $queryBenf=("select entes.nombre, estados_clientes.estado_cliente, beneficiarios.id_beneficiario where           
	      ...
	      titulares.id_cliente=clientes.id_cliente and clientes.cedula='$ceduclien'   
	      and estados_t_b.id_beneficiario=0  group by 
	      nombre,estado_cliente,id_beneficiario  order by estado_cliente");
	      $resuBenf=ejecutar($queryBenf);
	      $numfBenf=num_filas($resuBenf);
	      //ver si el titular esta como beneficiario de otro titular;
	      $queryTcomoB=("select entes.nombre, estados_clientes.estado_cliente, titulares.id_titular   where    
	                     titulares.id_ente=entes.id_ente and   
		             titulares.id_titular=estados_t_b.id_titular and 
			     estados_t_b.id_estado_cliente=estados_clientes.id_estado_cliente and 
	     ...
	     $numTcomoB=num_filas($resuTcomoB);
          ...
          $queryesBenf=("select clientes.*,beneficiarios.id_beneficiario,beneficiarios.id_titular where beneficiarios.id_cliente=clientes.id_cliente 
	                  and clientes.cedula='$ceduclien';");
	  ...
	  //ver a que ente pertenece y ver su estatus
          $queryEnteB=("select entes.nombre, estados_clientes.estado_cliente, titulares.id_titular, beneficiarios.id_beneficiario where    
	                 titulares.id_ente=entes.id_ente and 
		         titulares.id_titular=estados_t_b.id_titular and               
			 estados_t_b.id_estado_cliente=estados_clientes.id_estado_cliente and 
			 ...
			 clientes.cedula='$ceduclien' and estados_t_b.id_beneficiario=beneficiarios.id_beneficiario and 
		         titulares.id_titular=beneficiarios.id_titular group by nombre,estado_cliente,id_titular,id_beneficiario order by  
		         estado_cliente;");
	  $resuEnteB=ejecutar($queryEnteB);
	  $resuEntB1=("select entes.nombre, estados_clientes.estado_cliente, titulares.id_titular, beneficiarios.id_beneficiario where    
	               titulares.id_ente=entes.id_ente and 
		       titulares.id_titular=estados_t_b.id_titular and               
		       estados_t_b.id_estado_cliente=estados_clientes.id_estado_cliente and 
		       beneficiarios.id_cliente=clientes.id_cliente and            
   ...
   if ($numfTitular>0){
      ...
      ?>
		...
		<td class="tdcampos" ><?php echo $elidcliente['telefono_hab']; ?></td>
	     ...
	     echo "
	       <tr>
	         ...
	         <td class=\"tdcampos\" >$estaente[nombre]</td>";
	          ...
	          $querydiv=("select * from subdivisiones where titulares_subdivisiones.id_subdivision=subdivisiones.id_subdivision 
and titulares_subdivisiones.id_titular=$elttl1");
		 ...
		 <tr>     
	      <input  TYPE=\"hidden\" name=\"idcott\" id=\"idcott_$ti\" VALUE=$elttlar >  
              <td class=\"tdtitulos\"><input type=\"radio\" id=\"group_$ti\" value=$elttlar onClick=\"Effect.Grow('titcober');VerCob1()\" style=\"cursor:pointer\" title=\"Ver Cobertura\">Ver cobertura</td> </tr>";
		 $ti++;
		 $tv++;
		 }else{
                     echo"
	  ...
	  while ($dapBenfi=asignar_a($resuBenf)){   
	    ...
	    $queryDBf=("select clientes.nombres,clientes.apellidos,clientes.cedula,parentesco.parentesco where 
	...
	echo "<table class=\"tabla_cabecera3\" cellpadding=0 cellspacing=0>
		 ...
		 while ($dapTcB=asignar_a($resuTcomoB)){   
		   ...
		   $queryTcB=("select clientes.nombres,clientes.apellidos,clientes.cedula where 
	 ...
	 ?>
	   ...
	   <table class="tabla_cabecera5"  cellpadding=0 cellspacing=0>
		 ...
		 <td class="tdcampos" ><?php $daBenf['sexo'];
	      ...
	      <tr>
	           <td class="tdtitulos">Tel&eacute;fono:</td>
	           <td class="tdcampos" ><?php echo $daBenf['telefono_hab']; ?></td>
		   <td class="tdtitulos">Celular:</td>
	     ...
	     <tr>
	         <td colspan=1 class="tdtitulos">Comentarios:</td>
	         <td colspan=3 class="tdtitulos" ><label style="color: #ff0000"><?php echo $deBenf['comentarios']; ?></label></td>
	    </tr>
                 <?php
		 $ti=1;
		 $tv=1;
		       ...
		       $elttlar=$elttl1;
	               echo "
			         ...
			         <td class=\"tdcampos\" >$estaente[nombre]</td>";
	                     ...
	                     <tr>
                                <input  TYPE=\"hidden\" name=\"idcott\" id=\"idcott_$ti\" VALUE=$elttlar >  	   
                                <td class=\"tdtitulos\"><input type=\"radio\" id=\"group_$ti\" value=$elttlar onClick=\"Effect.Grow('titcober');VerCob1()\" style=\"cursor:pointer\" title=\"Ver Cobertura\">Ver cobertura</td>
		             </tr>";
			     $ti++;
			     $tv++;
		             }else{
		...
		echo"<table class=\"tabla_cabecera3\" cellpadding=0 cellspacing=0>
			   ...
			   $elidsoloB=$daTyB['id_beneficiario'];
		           $queryTcB=("select clientes.nombres,clientes.apellidos,clientes.cedula,entes.nombre,
			               ...
			               where titulares.id_titular=beneficiarios.id_titular and titulares.id_titular=$elideTyB and 
			               titulares.id_cliente=clientes.id_cliente and titulares.id_ente=entes.id_ente 
				       and estados_t_b.id_titular=titulares.id_titular 
			               and beneficiarios.id_parentesco=parentesco.id_parentesco and 
			               beneficiarios.id_beneficiario=$elidsoloB and
			               estados_clientes.id_estado_cliente=estados_t_b.id_estado_cliente 
				       group by nombres,apellidos,cedula,nombre,estado_cliente,parentesco order by estado_cliente;");
