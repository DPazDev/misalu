	...
	echo"<table class=\"tabla_cabecera3\"  cellpadding=0 cellspacing=0>
      ...
      for ($f=0;$f<$cuantos;$f++){
      	...
      	$iddependencia=$lamatriz[$f][4];
			...
			$pasogtb=1;
                $insumoalmacen=("select tbl_insumos_almacen.cantidad from tbl_insumos_almacen,tbl_dependencias where
                                tbl_insumos_almacen.id_insumo=$idinsumopro and tbl_insumos_almacen.id_dependencia=$iddependencia and
                ...
                $actualizaralmacen=("update tbl_insumos_almacen set cantidad=$lanuevacanti from tbl_dependencias where
                         tbl_insumos_almacen.id_insumo=$idinsumopro and tbl_insumos_almacen.id_dependencia=$iddependencia and
                         tbl_insumos_almacen.id_dependencia = tbl_dependencias.id_dependencia and
                         tbl_dependencias.activo <> 1;");
                $repactualmacen=ejecutar($actualizaralmacen);
								...
								$RegistroMovimientossql=("INSERT INTO tbl_insumos_movimientos (id_insumo,id_dependencia,id_proveedor,movimiento,tipo_movimiento,precio_unitario,cantida,cantidad_almacen,cantida_actual,id_admin,nota_movimiento)
																	VALUES ('$idinsumopro','$iddependencia','$provees','$Movimiento','$TipoMovimiento','$precioprodu','$cantidades','$cantiactual','$lanuevacanti','$elid','$Descripcion');");
...
$log="REGISTRO LA ORDEN NUMERO $elprocesocargado DEL ID TITULAR $estitu Y ID BENEFICIARIO $esbene COMENTARIO $comentarioes , TIPO SERVICIO $idtipservi Y TIPO SERVICIO $idservi ";
logs($log,$ip,$elid);
