...
<tr>
<td colspan=2>
                <select style="width: 200px;" name="proveedor" id="proveedor-servicio" class="campos" onChange="reg_oa2_proveedor(this);">
                <?php
 if($ClasifiaProveedor=='0' || $ClasifiaProveedor=='1') {
 	//filtrar por clasificacion si son intramurales o extramurales
 $q_pc=("select clinicas_proveedores.*,proveedores.id_proveedor
...
else
{
		if(($tiposerv==25)){
			?>
			<td colspan=2 class="tdtitulos"><a href="#" OnClick="guardareme('reg_oa2');" class="boton">Guardar</a></td>
			<?php
			}
				else{
						?>
