...
if($cuantoshay==0){
	...
	$buscarToB=("select titulares.id_titular,titulares.id_ente,entes.nombre
                            from titulares,estados_t_b,entes
                           where
	...
	<?php
              ...
              while($grupoarti=asignar_a($buscarinforglo,NULL,PGSQL_ASSOC)){
				if($estitu>1){
		         ...
		         echo"</select>
                          ...
                          <tr>
                          <td class=\"tdcampos\"><a href=\"views01/farmaext3.php?tipoc=$esun&idbusq=$parbuscarid&tipooper=GC\" title=\"Ver los gastos del $ventamensaje\" class=\"boton\" onclick=\"Modalbox.show(this.href, {title: this.title, width:900,height:400, overlayClose: false}); return false;\">Gastos Clientes</a></td>
                          <td class=\"tdcampos\"><a href=\"views01/farmaext3.php?tipoc=$esun&idbusq=$parbuscarid&tipooper=TC\" title=\"Ver los tratamientos continuo del $ventamensaje\" class=\"boton\" onclick=\"Modalbox.show(this.href, {title: this.title, width:900,height:400, overlayClose: false}); return false;\">Tratamiento Continuo</a></td> ";
			...
			if($esTyB==1){
		         ...
		         echo"</select>
                          ...
                          <tr>
                             <td class=\"tdcampos\"><a href=\"views01/farmaext3.php?tipoc=B&idbusq=$grupoartis[id_beneficiario]&tipooper=GC\" title=\"Ver los gastos del Beneficiario(a) $nomcompleto registrado al titular $nomcompletCIT\" class=\"boton\" onclick=\"Modalbox.show(this.href, {title: this.title, width:900,height:400, overlayClose: false}); return false;\">Gastos Clientes</a></td>
                              <td class=\"tdcampos\"><a href=\"views01/farmaext3.php?tipoc=B&idbusq=$grupoartis[id_beneficiario]&tipooper=TC\" title=\"Ver los tratamientos continuo del Beneficiario(a) $nomcompleto registrado al titular $nomcompletCIT\" class=\"boton\" onclick=\"Modalbox.show(this.href, {title: this.title, width:900,height:400, overlayClose: false}); return false;\">Tratamiento Continuo</a></td>";
				echo "<tr><td colspan=7><hr></td></tr>";
				$tb++;
				}
	...
	?>
	  <tr>
          ...
          <tr>
            ...
            <td class="tdcampos"><input type="text" id="laenfermedad" class="campos" size="35"></td>
          </tr>
          <tr>
            <td class="tdtitulos">Comentario operador:</td>
            <td class="tdcampos"><TEXTAREA COLS=35 ROWS=3 id="comentope" class="campos"></TEXTAREA></td>
          </tr>
		  <?
				...
				echo"<tr>
                        <td class=\"tdtitulos\">Proveedor:</td>
                        <td class=\"tdcampos\">
                        ...
                        if($opera1=='externa'){?>
			  	                    <option value="698">FARMA EXPRESS</option>
			  	                                <?}?>
                              <option value=""></option>
                              <option value="180">FARMACIA CLINISALUD-MERIDA</option>
                   			  	  <option value="238">FARMACIA CLINISALUD EL VIGIA </option>
			                <?while($losproveedores=asignar_a($repbuscaproveedores,NULL,PGSQL_ASSOC)){
					                   echo"<option value=\"$losproveedores[id_proveedor]\">$losproveedores[nombre]</option>";
			          }
...
?>
     ...
     <input type="hidden" id="laopera"  value="<?echo $opera1?>">
          <tr>
            <td  title="Procesar orden de m&eacute;dicamento externa"><label class="boton" style="cursor:pointer" onclick="procesarmEX(); return false; " >Procesar</label></td>
          </tr>
