...
//verificar que la variable activos este seteada
if(isset($_POST['activar'])) {
	$activar=$_POST['activar'];
	if($activar=='si') {
	$chequesi='checked="checked"';
	$chequeno='';
	...
	{
		$chequesi='';
	$chequeno='checked="checked"';
		$act=false;
		$activos='and activar=1';
		$activs='no';
	}	
...
¿Quieres ver inactivos?: 
Si<input type="radio" name='activos' id="activossi" value="true"  <?php echo $chequesi; ?> onclick="activarcon_morb('si');">
No<input type="radio" name='activos' id="activosno" value="false" <?php echo $chequeno; ?> onclick="activarcon_morb('no');">
...
<td colspan=2 class="tdcampos">
		  <select style="width: 200px;" OnChange="verificarproc(this);" id="id_proveedorc" name="id_proveedorc" class="campos">
                ...
                $activos order by clinicas_proveedores.nombre");
                $r_pc=ejecutar($q_pc);
