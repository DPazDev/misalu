<?php
 ...
 $fecha=date("Y-m-d");
   $tipcliente=$_REQUEST["eltipclien"];
  if($tipcliente==1){
       $querytente="and (entes.id_tipo_ente<>4 and entes.id_tipo_ente<>6)";
    }else{
  ...
  $numfilas=num_filas($resuCedula);
    ...
    $queryTitular=("select titulares.* from titulares,entes where titulares.id_cliente=$vercliente and
    ...
    $numfTitular=num_filas($resuTitular);
       if ($numfTitular>0){
	     ...
	     //ver a que ente pertenece y ver su estatus
	       $queryEnte=("
select entes.nombre, entes.id_ente, estados_clientes.estado_cliente,titulares.id_titular,titulares.codigo_empleado,
...
entes.id_tipo_ente=tbl_tipos_entes.id_tipo_ente $querytente order by estado_cliente;");
	       $resuEnte=ejecutar($queryEnte);
              //busco a ver si tiene beneficiarios y su entes y su estatus
	      $queryBenf=("select 
    entes.nombre, estados_clientes.estado_cliente, beneficiarios.id_beneficiario,titulares.id_titular
from
...
where           
   ...
   estados_t_b.id_estado_cliente=estados_clientes.id_estado_cliente $querytente;");
...
from beneficiarios,clientes,titulares,entes
where beneficiarios.id_cliente=clientes.id_cliente 
	                  and clientes.cedula='$ceduclien' and
      ...
      ?>
        ...
        <table class="tabla_citas"  cellpadding=0 cellspacing=0>
           <tr>
                ...
                <td colspan=3 class="tdcampos" ><label style="color: #ff0000; font-size: 17pt"><?php echo "$elidcliente[comentarios]"; ?></label></td>
            </tr>
            <tr><td class="tdtitulos" colspan=7><hr></td></tr>
        </table> 
	    ...
	    $elttlar=$elttl1;
            $elenteq=$estaente[id_ente];
            $verpartidad=("select tbl_partidas.tipo_partida from tbl_partidas,titulares 
	                where titulares.tipo_partida=tbl_partidas.id_partida and titulares.id_titular=$elttl1;");
	    $repverpartida=ejecutar($verpartidad);   
	    $datdepartida=assoc_a($repverpartida);      
	    $lapartida=$datdepartida['tipo_partida'];  
		$url="'views01/isolicitudmedicamento.php?id_titular=$elttlar&id_beneficiario=0'";
	     echo "
                <tr>
	         <td class=\"tdtitulos\">Partida:</td>
	         <td class=\"tdcampos\" >$lapartida</td>
               </tr> 
	       <tr>
...
and titulares_subdivisiones.id_titular=$elttl1");
		  ...
		  $datadiv=assoc_a($resuldiv);
                  $verpoliza=("select polizas.nombre_poliza,polizas.deducible from titulares_polizas,polizas 
                               where titulares_polizas.id_titular=$elttl1 and
                               titulares_polizas.id_poliza=polizas.id_poliza and
                               polizas.particular=1 order by polizas.deducible desc");
                  $repvepoliza=ejecutar($verpoliza);
                  $cuantaspoliza=num_filas($repvepoliza);
                  $numcontrato=("select tbl_contratos_entes.numero_contrato,tbl_recibo_contrato.num_recibo_prima,
                                tbl_recibo_contrato.fecha_ini_vigencia,tbl_recibo_contrato.fecha_fin_vigencia  
                                from 
                                 tbl_contratos_entes,entes,titulares,tbl_recibo_contrato
                                where
                                 titulares.id_titular=$elttl1  and
                                 titulares.id_ente=entes.id_ente and
                                 entes.id_ente=tbl_contratos_entes.id_ente and
                                 tbl_contratos_entes.id_contrato_ente=tbl_recibo_contrato.id_contrato_ente order by 
                                 tbl_contratos_entes.id_contrato_ente;");

                  $repnumcontr=ejecutar($numcontrato);
                  $repnumcontr1=ejecutar($numcontrato);
                  ...
                  $cuantdesa=num_filas($repuesta);
                    ...
                    <td  class=\"tdtitulos\" ><label style=\"color: #ff0000\"> $estaente[codigo_empleado]</label></td>
                       ...
                       if($cuantaspoliza>=1){
                       $dataverpoliza=assoc_a($repvepoliza);
                       $nombpoliza=$dataverpoliza[nombre_poliza];
                       $eldeducible=$dataverpoliza[deducible];
                       if($eldeducible==NULL){
                           $eldeducible=0;
                           }
                         echo"
                        ...
                        <td class=\"tdtitulos\">Poliza:</td>
                            <td  class=\"tdtitulos\" ><label style=\"color: #ff0000\">$nombpoliza</label></td>
                       <td class=\"tdtitulos\">Deducible:</td>
                            <td  class=\"tdtitulos\" ><label style=\"color: #ff0000\">$eldeducible</label></td></tr>
                       <tr>";?>
                       <input type="hidden" id="cedula" value="<?echo $ceduclien?>">
                       <input type="hidden" id="numero_contrato" value=""> 
                          <td class="tdtitulos">No. Contrato:</td>
...
?>
		 ...
		 <tr>"; 
           $buscamotomador=("select * from coberturas_t_b where id_titular=$elttl1 and id_beneficiario=0;");
	   $repcuantomador=ejecutar($buscamotomador);
	   $cantidtomador=num_filas($repcuantomador);
	   if($cantidtomador==0){
           ?>
            <tr>
		 <td class="tdtitulos">Tipo de Titular:</td>
	         <td class="tdcampos" ><label style="color: #ff0000"><h1>Tomador</h1></label></td>
	    </tr>  
           <?
              }
              ...
              $buscahitcomen=("select count(registros_exclusiones.id_titular) from registros_exclusiones where 
                                                id_titular=$elttl1 and id_beneficiario=0;");
                $repbushist=ejecutar($buscahitcomen);
                $datahist=assoc_a($repbushist);
                $cuantoshay=$datahist['count'];
                if($cuantoshay>=1){
                      echo "
                         <td class=\"tdtitulos\">
                           <a href=\"views01/historialcoment.php?titular=$elttl1-T\" title=\"Ver historial de comentarios en el titular $elidcliente[nombres] $elidcliente[apellidos]\" class=\"boton\" onclick=\"Modalbox.show(this.href, {title: this.title, width:900,height:400, overlayClose: false}); return false;\">Historial Comentario</a>
                      </td>";
                    }
         }else{
               ...
               $buscahitcomen=("select count(registros_exclusiones.id_titular) from registros_exclusiones where 
                ...
                if($cuantoshay>=1){
                      echo "
                         <td class=\"tdtitulos\">
                           <a href=\"views01/historialcoment.php?titular=$elttl1-T\" title=\"Ver historial de comentarios $elidcliente[nombres] $elidcliente[apellidos]\" class=\"boton\" onclick=\"Modalbox.show(this.href, {title: this.title, width:900,height:400, overlayClose: false}); return false;\">Historial Comentario</a>
                      </td>";
                    }
               ?>
...
$veranexo=("select tbl_anexos_repuesta.id_anexo from tbl_anexos_repuesta where id_titular=$elttl1;");
$repveranexo=ejecutar($veranexo);
$cuantanexo=num_filas($repveranexo);
if($cuantanexo>=1){
 $dataanexo=assoc_a($repveranexo);	
 $elanexoid=$dataanexo[id_anexo];
 echo "<td class=\"tdtitulos\">
      <a href=\"views01/mostaranexos.php?anexoid=$elanexoid\" title=\"Planilla Anexo\" class=\"boton\" onclick=\"Modalbox.show(this.href, {title: this.title, width:900,height:400, overlayClose: false}); return false;\">Anexo</a>
     </td>";
}
echo"</tr>
<tr>
		 ...
		 $tv++;
                 ...
                 $contanulado=("select tbl_contratos_entes.numero_contrato,
                               tbl_contratos_entes.id_contrato_ente,tbl_contratos_entes.fechanulacion,
                               tbl_contratos_entes.id_ente,polizas.nombre_poliza
                                from 
                                 tbl_contratos_entes,entes,titulares,polizas,polizas_entes
                                where
                                 entes.id_ente=$elenteq and
                                 titulares.id_ente=entes.id_ente and
                                 entes.id_ente=tbl_contratos_entes.id_ente and
                                 tbl_contratos_entes.estado_contrato='0' and
                                 polizas_entes.id_ente=entes.id_ente and
                                 polizas_entes.id_poliza=polizas.id_poliza group by               
                                 tbl_contratos_entes.numero_contrato,
                                 tbl_contratos_entes.id_contrato_ente,tbl_contratos_entes.fechanulacion,
                                 tbl_contratos_entes.id_ente,polizas.nombre_poliza      order by 
                                 tbl_contratos_entes.id_contrato_ente;");  
                   $repcontaanulado=ejecutar($contanulado);
                   $dataanulado=assoc_a($repcontaanulado);
                   $idcontente=$dataanulado['id_contrato_ente'];
                   $numecontrato=$dataanulado['numero_contrato'];
                   $lapoliza=$dataanulado['nombre_poliza'];
                   if($idcontente>=1){
					   $recibocontr=("select tbl_recibo_contrato.num_recibo_prima from 
					                    tbl_recibo_contrato where id_contrato_ente=$idcontente order by fecha_creado desc limit 1");
					   $reprecibocontr=ejecutar($recibocontr);
					   $datarecibcont=assoc_a($reprecibocontr);
					   $numrecibo=$datarecibcont[num_recibo_prima];
					   $elcontrato="$numecontrato | No. Recibo: $numrecibo";
				   }   
                     echo"
                           <td class=\"tdtitulos\">Estatus:</td>
                ...
                if($idcontente>=1){   
                    echo" 
                     <tr>
                     <td class=\"tdtitulos\">No. Contrato:</td>
                      <td class=\"tdtitulos\">$elcontrato</td>
                      <td class=\"tdtitulos\">Poliza:</td>
                      <td class=\"tdtitulos\">$lapoliza</td>
                     </tr>
                     "; 
                  }
                $buscahitcomen=("select count(registros_exclusiones.id_titular) from registros_exclusiones where 
...
where 
	   ...
	   $ladatBF=assoc_a($resuDBf); 
           ...
           echo "
	        <tr>
	              <td class=\"tdcampos\" onclick=\"hola('$lacedut','$elidtitu')\" style=\"cursor:pointer\" title=\"Ver data\"> $ladatBF[cedula]</td>
	...
	echo"</table>";
	 ...
	 ?>
           ...
           <tr>
                <td colspan=1 class="tdtitulos">Comentarios:</td>
                <td colspan=3 class="tdtitulos" ><label style="color: #ff0000; font-size: 17pt "><?php echo $daBenf['comentarios']; ?></label></td>
            </tr>
            <tr><td class="tdtitulos" colspan=7><hr></td></tr>
        </table> 
	   <table class="tabla_cabecera5"  cellpadding=0 cellspacing=0>
		 ...
		 $tv=1;
                ...
                if($cuantoshay>=1){
                      echo "
                         <td class=\"tdtitulos\">
                           <a href=\"views01/historialcoment.php?titular=$elttl1-B\" title=\"Ver historial de comentarios  $daBenf[nombres] $daBenf[apellidos]\" class=\"boton\" onclick=\"Modalbox.show(this.href, {title: this.title, width:900,height:400, overlayClose: false}); return false;\">Historial Comentario</a>
                      </td>";
                    }
            ...
            <? }
$veranexo=("select tbl_anexos_repuesta.id_anexo from tbl_anexos_repuesta where id_beneficiario=$elttl1;");
$repveranexo=ejecutar($veranexo);
$cuantanexo=num_filas($repveranexo);
if($cuantanexo>=1){
 $dataanexo=assoc_a($repveranexo);	
