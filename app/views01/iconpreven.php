...
<p class="titulo_seccion">Relacion de Consultas Preventivas <?php echo $textfechainicio ?> hasta <?php echo $textfechafinal ?>&nbsp;</p>
 ...
 else
{
	...
	$r_clienteb=pg_query("select clientes.id_cliente,clientes.apellidos,clientes.nombres,clientes.cedula,beneficiarios.id_beneficiario,beneficiarios.id_titular from clientes,beneficiarios where clientes.cedula='$cedula' and clientes.id_cliente=beneficiarios.id_cliente and beneficiarios.id_titular=titulares.id_titular and beneficiarios.id_titular=$id_titular");
	($f_clienteb=pg_fetch_array($r_clienteb, NULL, PGSQL_ASSOC));
	?>
 <?php 
	$r_clientetitular=pg_query("select clientes.id_cliente,clientes.apellidos,clientes.nombres,clientes.cedula,titulares.id_titular,entes.nombre,entes.id_ente from clientes,titulares,entes where clientes.id_cliente=titulares.id_cliente and titulares.id_ente=entes.id_ente and beneficiarios.id_titular=titulares.id_titular and beneficiarios.id_beneficiario=$f_clienteb[id_beneficiario]");
...
from procesos,gastos_t_b,estados_procesos where procesos.id_titular=$f_clientet[id_titular] and procesos.id_beneficiario=0 and
...
<?php 
  $r_proveedor_persona=pg_query("select * from personas_proveedores where personas_proveedores.id_persona_proveedor=s_p_proveedores.id_persona_proveedor and s_p_proveedores.id_s_p_proveedor=proveedores.id_s_p_proveedor and  proveedores.id_proveedor=$f_gastos_t_b[id_proveedor]");
  ($f_proveedor_persona=pg_fetch_array($r_proveedor_persona, NULL, PGSQL_ASSOC));
  ?>

  <?php 
  $r_proveedor_clinica=pg_query("select * from clinicas_proveedores where clinicas_proveedores.id_clinica_proveedor=proveedores.id_clinica_proveedor and  proveedores.id_proveedor=$f_gastos_t_b[id_proveedor]");
...
$r_gastos_t_b1=pg_query("select admin.nombres,procesos.id_proceso,servicios.servicio,procesos.fecha_recibido,
	gastos_t_b.id_cobertura_t_b,estados_procesos.estado_proceso,procesos.comentarios,count(gastos_t_b.id_proceso) 
from procesos,gastos_t_b,estados_procesos where procesos.id_titular=$f_clientetitular[id_titular] and procesos.id_beneficiario=$f_clienteb[id_beneficiario]  and
 servicios.id_servicio=$servicio and procesos.fecha_recibido>='$textfechainicio'
 and procesos.fecha_recibido<='$textfechafinal' and gastos_t_b.id_proceso=procesos.id_proceso and
 gastos_t_b.id_servicio=servicios.id_servicio and
 gastos_t_b.id_tipo_servicio=5
