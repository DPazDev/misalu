...
$q_morbilidad=("select entes.id_tipo_ente,entes.nombre,clientes.nombres,clientes.apellidos,clientes.cedula,clientes.edad,clientes.sexo,clientes.fecha_nacimiento,procesos.id_proceso,procesos.fecha_recibido,procesos.comentarios,admin.nombres as adminnombre,procesos.id_titular,procesos.id_beneficiario,gastos_t_b.fecha_cita,gastos_t_b.enfermedad from clientes,procesos,gastos_t_b,proveedores,s_p_proveedores,personas_proveedores where gastos_t_b.id_proceso=procesos.id_proceso and gastos_t_b.id_proveedor=proveedores.id_proveedor and gastos_t_b.id_proveedor='$proveedor' and gastos_t_b.fecha_cita>='$fechainicio' and gastos_t_b.fecha_cita<='$fechafin' and clinicas_proveedores.id_clinica_proveedor=proveedores.id_clinica_proveedor and procesos.id_titular=titulares.id_titular and titulares.id_cliente=clientes.id_cliente and titulares.id_ente=entes.id_ente and procesos.id_admin=admin.id_admin
");
$r_morbilidad=ejecutar($q_morbilidad);


$q_persona=("select clinicas_proveedores.*,proveedores.id_proveedor from clinicas_proveedores,proveedores where clinicas_proveedores.id_clinica_proveedor=proveedores.id_clinica_proveedor and proveedores.id_proveedor=$id_proveedor");
...
align="center" class="titulo_seccion">Morbilidad del  <?php echo "$f_persona[nombres]  "?> Fecha Cita: <?php 		if ($fechainicio==$fechafin ){
																					echo $fechainicio;
																					}
																					else{
