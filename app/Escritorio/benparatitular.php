...
$datosprincipales=("select * from clientes where cedula='$cedclien';");
$repdatosprin=ejecutar($datosprincipales);
$dapaprin=assoc_a($repdatosprin);
$elidclientes=$dapaprin['id_cliente'];
...
$cuantpermiso=num_filas($repbuscarpermis);
if($cuantpermiso>=1){
$losentes=("select entes.id_ente,entes.nombre from entes order by entes.nombre");
$reslosentes=ejecutar($losentes);
}else{
  $losentes=("select entes.id_ente,entes.nombre from entes where (entes.id_tipo_ente=4 or entes.id_tipo_ente=6 or entes.id_tipo_ente=8) order by entes.nombre");
  $reslosentes=ejecutar($losentes);
}
...
<table class="tabla_cabecera5"  cellpadding=0 cellspacing=0>
	 ...
	 <tr>
       ...
       <td class="tdcampos"  colspan="1"><select id="clienente" class="campos" onChange="verpoliza()" style="width: 260px;">
			   ...
			   <?php  while($verente=asignar_a($reslosentes,NULL,PGSQL_ASSOC)){?>
                              <option value="<?php echo $verente[id_ente]?>"> <?php echo "$verente[nombre]"?></option>
			   <?php
			  }
			  ?>
			 </select> 
	...
	<tr>
	   <td class="tdtitulos" colspan="1">Nombre:</td>
        <td class="tdcampos"  colspan="1"><input type="text" id="cliennombre" class="campos" size="30" value="<?echo $dapaprin['nombres']?>"></td>
       <td class="tdtitulos" colspan="1">Apellido:</td>
       <td class="tdcampos" colspan="1"><input type="text" id="clienapellido"  class="campos" size="30" value="<?echo $dapaprin['apellidos']?>"></td>
	 </tr>   
	<tr>
       ...
       <td class="tdcampos" colspan="1"><input type="text" id="cliencorre"  class="campos" size="30" value="<?echo $dapaprin['email']?>"></td>
	 </tr>    
	<tr>
	    <td class="tdtitulos" colspan="1">Tel&eacute;fono habitaci&oacute;n:</td>
       <td class="tdcampos" colspan="1"><input type="text" id="clientf1"  class="campos" size="15" value="<?echo $dapaprin['telefono_hab']?>"></td>				
       <td class="tdtitulos" colspan="1">Tel&eacute;fono celular:</td>
       <td class="tdcampos" colspan="1"><input type="text" id="clientf2"  class="campos" size="15"></td>
	 </tr>  
	 <tr>
	    <td class="tdtitulos" colspan="1">Fecha de nacimiento:</td>
       <td class="tdcampos" colspan="1"><input  type="text" size="10" id="fechanaci" class="campos" maxlength="10" value="<?echo $dapaprin['fecha_nacimiento']?>">
	...
	<tr> 
       ...
       <td class="tdcampos" colspan="3"><TEXTAREA COLS=60 ROWS=2 id="cliendirr" class="campos"><?echo $dapaprin['direccion_hab']?></TEXTAREA></td>
	</tr> 
	<tr>
	...
	<br>
	     ...
	     <tr>
	       <td title="Guardar cliente"><label id="benparatitu" class="boton" style="cursor:pointer" onclick="validclienteB()" >Guardar</label>
	      </tr>
          </table>	 
</div>
