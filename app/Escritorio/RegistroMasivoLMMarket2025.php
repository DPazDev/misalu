...
<!-- UTILIZAR FORMATO DE FECHA: DD-MM-YY                          -->
<!-- SOLO PARA LAS FECHAS DE NACIMIENTO EN EL EXCEL               -->
<!-- EJEMPLO: BD_LM_MARKET                                        -->
<!-- (INDICACIÓN DE DANIEL PAZ - AÑO 2025)                        -->
<!--⚠️ ============================================================= ⚠️-->
<!-- !!! CUIDADO CON FECHAS DE NACIMIENTO !!!                     -->
<!-- FECHAS ANTES DE 1969 PUEDEN SER INTERPRETADAS COMO 2069 EN PHP -->
<!-- VERIFICAR Y CORREGIR MANUALMENTE SI ES NECESARIO             -->
<!-- ============================================================= -->
<!-- ============================================================= -->
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
///EL ENTE
		$entclien='3383';//ID ENTE LABORATORIOS LYA CA
		$poliza='37';//ID PLAN CLINI LABORAL PLUS MEDICINA OCUPACIONAL
    ///Cambio de plan: renueva el y traslada todas las cualidades a la nueva
    $TrasladoPlan='0';////si desea cambiar de plan id de plan anterior
		$fecha=date("Y-m-d");
		$hora=date("H:i:s");
///contadores
		$clientesnuevos=0;
...
///primer valor explotado 0 DIA
   ...
   //TERCER VALOR explotado año 2
			...
			$valor = trim($data->sheets[0]['cells'][$i][6]);

			if (!empty($valor)) {
				// Reemplazamos / por - para evitar líos con strtotime y DateTime
				$valor_formateado = str_replace('/', '-', $valor);
			
				// Detectamos cuántos dígitos tiene el año
				$partes = explode('-', $valor_formateado);
				$anio = isset($partes[2]) ? $partes[2] : null;
			
				if ($anio !== null && strlen($anio) == 4) {
					$fechaObj = DateTime::createFromFormat('d-m-Y', $valor_formateado);
				} else {
					$fechaObj = DateTime::createFromFormat('d-m-y', $valor_formateado);
				}
			
				if ($fechaObj !== false) {
					$fechaObj->modify('+1 day'); // Sumar el día
					$fnaciclien = $fechaObj->format('Y-m-d');
					echo "xd $fnaciclien";
				} else {
					echo "Error: La fecha <strong>$valor</strong> no se pudo convertir correctamente";
					$fnaciclien = null;
				}
			} else {
				echo "⚠️ Advertencia: No se encontró una fecha en la celda";
				$fnaciclien = null;
			}
    ...
    { //CONSULTA DE Propiedades poliza
      ...
      while($laspolizason=asignar_a($respupropolizas,NULL,PGSQL_ASSOC)){
            ...
            $BuscarCoberturaCulid=("select id_cobertura_t_b,propiedades_poliza.id_propiedad_poliza,propiedades_poliza.cualidad,propiedades_poliza.monto_nuevo from propiedades_poliza,coberturas_t_b where
                      ...
                      coberturas_t_b.id_titular='$elidtitulares' and id_beneficiario=0 and
