...
$filename = "BASE DE DATOS EPSD JULIO 2020.xls";
$error=0;
	...
	echo "<h1><br> CLEINTE $registroData <br>-------------------------------------<br></h1>";
             ...
             //Fechas Exel
              ...
              $separador='/';
               ...
               //echo "<h1>DIA: $DIA -$f[2] Mes $MES -$f[1] AÑO $AAA-$f[0]  ---- $fechaFormato<br></h1>";
              $fech=$f[0].'-'.$f[1].'-'.$f[2];
					$fecha2='50-04-16';              
              $date = new DateTime($fecha2);
              $fechaF1=$date->format('Y-m-d H:i:s');
   			         
            
              $fech2=date('d-m-Y',strtotime($fecha2."+ 1 day")); 
         //     echo "<h1>FECHA: $fechaF1-----$fecha_nac2 --------F2 $fech2<br></h1>";
				  ...
				  $fnaciclien2=date('d-m-Y',strtotime('+1 day',$fecha_nac2));    
		//		  echo "<h1>FECHA2: ($fecha_nac22) =$fecha_nac3    fr $fnaciclien2<br></h1>";     
              $fnaciclien=date('d-m-Y',strtotime('+1 day',$fecha_nac3));//fecha de Nacimiento
              
              $edad=$data->sheets[0]['cells'][$i][7];//Edad
              ...
              $comentclien=$data->sheets[0]['cells'][$i][17];
              ...
              //tratar Fecha Exell
              	$ingresoclienexell=$data->sheets[0]['cells'][$i][21]; //FECHA DE INGRESO A LA EMPRESA
					$ingresoclien2 = ($ingresoclienexell - 25569) * 86400;              
              	$ingresoclien=date('d-m-Y',strtotime('+1 day',$ingresoclien2));//FECHA DE INGRESO A LA EMPRESA            
              //tratar Fecha Exell 
...
if($numversisotitu<=0){
		...
		$bustituclien=("select id_titular from titulares where id_cliente='$elidcliente';");
...
echo "<h3>ESTADOS TB<br></h3>";
 ...
 echo "<h3>Primer 8 ACTUALIZAR TITULAR POLIZA $poliza<br></h3>";
			...
			//Buscar el cliente guardado en la tabla titulares
			echo$bustituclien=("select id_titular from titulares where titulares.id_cliente=$elidcliente and titulares.id_ente=$entclien ;");
			$resptituclien=ejecutar($bustituclien);
			$dattituclien=assoc_a($resptituclien);
			$elidtitulares=$dattituclien['id_titular'];
