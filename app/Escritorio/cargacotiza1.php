...
$nomtitu=strtoupper($_POST[nombtitu]);
$apelltitu=strtoupper($_POST[apelltitu]);
$elgenetitu=$_POST[generotitu];
$nacimtitu=$_POST[fechanactitu]; 
...
//ver si el titular ya esta en la tabla titulares_polizas
$versiyatiene=("
select titulares_polizas.id_poliza,polizas.nombre_poliza 
from
 titulares_polizas,titulares,clientes,polizas
where
 titulares.id_cliente=clientes.id_cliente and
 clientes.cedula='$titulcedula' and
 titulares.id_titular=titulares_polizas.id_titular and
 titulares_polizas.id_poliza=$polizatitular and
 titulares_polizas.id_poliza=polizas.id_poliza;");

$repvesitien=ejecutar($versiyatiene);
$cuantsitien=num_filas($repvesitien); 
if($cuantsitien>=2){
...
<table class="tabla_cabecera3" cellpadding=0 cellspacing=0>
    <tr>
	  <td colspan=4 class="titulo_seccion"><label style="color: #ff0000"><h1>El titular ya esta registrado en la poliza <?echo $nopolisih?>!!!</h1></label></td>
       </tr>
</table>
<?}else{
...
$cuantcliexis=num_filas($repbusclienexis);
  if($cuantcliexis==0){//cliente NO EXISTE!!!
     //guardamos el cientes en la tabla entes
     $guardente=("insert into entes(nombre,telefonos,direccion,rif,email_contacto,fecha_creado,hora_creado,
                                                        id_ciudad,fecha_inicio_contrato,fecha_renovacion_contrato,
                                                        id_sucursal,id_tipo_ente,fecha_inicio_contratob,fecha_renovacion_contratob,
                                                        es_med_pre) values('$nomtitu $apelltitu','$teleftitu','$direcctitu','$titulcedula',
                                                        '$elcorretitu','$fecha','$hora',$ciudadtitu,'$iniciocotrato','$findecotrato',
                                                        $lasucursal,7,'$iniciocotrato','$findecotrato',1);");
     $repguaente=ejecutar($guardente);     
     //buscamos el id_ente que se genera
      $selecidente=("select entes.id_ente,entes.id_tipo_ente,entes.fecha_creado from entes where entes.rif='$titulcedula' and fecha_creado='$fecha' and
                                                                                                                            entes.hora_creado='$hora';");
     ...
     //guardamos al nuevo cliente en la tabla clientes
     $nuevcliente=("insert into clientes(nombres,apellidos,fecha_nacimiento,sexo,direccion_hab,telefono_hab,
                                        celular,email,fecha_creado,hora_creado,id_ciudad,comentarios,cedula,id_admin,edad) 
                                        values('$nomtitu','$apelltitu','$nacimtitu','$elgenetitu','$direcctitu','$teleftitu','$teleftitu',
                                        '$elcorretitu','$fecha','$hora',$ciudadtitu,'$comenttitu','$titulcedula',$elid,$edadtitu);");
     
     $repnuevcliente=ejecutar($nuevcliente);      
     //buscamos el cliente que guardamos para saber el id_cliente
     $busidcliente=("select clientes.id_cliente from clientes where clientes.cedula='$titulcedula' and clientes.fecha_creado='$fecha';");
     $repbuscliente=ejecutar($busidcliente);                                       
     $datidcliente=assoc_a($repbuscliente);
     $elcienteides=$datidcliente[id_cliente];//--------------->ya tenemos el id_cliente
     ...
     $guardartitu=("insert into titulares(id_cliente,fecha_ingreso_empresa,fecha_creado,hora_creado,
                                        id_ente,fecha_inclusion,id_admin,maternidad,tipocliente) 
                              values($elcienteides,'$fechaincltitu','$fecha','$hora',$elidentees,'$fechaincltitu',$elid,1,$eltipti);");
     }else{
            $guardartitu=("insert into titulares(id_cliente,fecha_ingreso_empresa,fecha_creado,hora_creado,
                                        id_ente,fecha_inclusion,id_admin,maternidad,tipocliente) 
                              values($elcienteides,'$fechaincltitu','$fecha','$hora',$elidentees,'$fechaincltitu',$elid,0,$eltipti);");
         }
        $repguartitu=ejecutar($guardartitu);
        //una vez registrado el titular buscamos cual es el id_titular
    $buscidtitu=("select titulares.id_titular from titulares where titulares.id_cliente=$elcienteides;");    
    ...
    $guardtituestatb=("insert into estados_t_b(id_estado_cliente,id_titular,id_beneficiario,fecha_creado,hora_creado) 
                                              values($titulaestado,$eliddeltitu,0,'$fecha','$hora');");
    ...
    $guartituregexclu=("insert into registros_exclusiones(fecha_inclusion,fecha_exclusion,id_titular,id_beneficiario,
                                      fecha_creado,id_estado_cliente,comentario,id_admin) values('$fecha','$fecha',$eliddeltitu,0,
                                      '$fecha',$titulaestado,'$comenttitu',$elid);");
    ...
    //ahora guardamos el ente en la tabla polizas_entes
      $guapoliente1=("insert into polizas_entes(id_ente,id_poliza) values($elidentees,$polizatitular);");
    ...
    //ahora guardamos el ente en la tabla polizas_entes
      $guapoliente=("insert into polizas_entes(id_ente,id_poliza) values($elidentees,$polizamatertitu);");
    ...
    //buscamos cuantas poliza tiene el titular para cargar las coberturas_t_b
      ...
      $repbuspolititu=ejecutar($buslapolititu);
       while($lapoliza=asignar_a($repbuspolititu,NULL,PGSQL_ASSOC)){//se comienza a guardar en coberturas_t_b
              ...
              $buscpropipoliza=("select propiedades_poliza.id_propiedad_poliza,propiedades_poliza.monto,
                                               propiedades_poliza.organo 
                                             from propiedades_poliza where propiedades_poliza.id_poliza=$idpolies");
               $repbuspropoliza=ejecutar($buscpropipoliza);                              
               while($propoliza=asignar_a($repbuspropoliza,NULL,PGSQL_ASSOC)){
				 //**CATEEM**//  
				  if($quecliente==1){				     
                   $guarcobtb=("insert into coberturas_t_b(id_propiedad_poliza,id_titular,id_beneficiario,
                                           fecha_creado,hora_creado,id_organo,monto_actual,monto_previo) 
                                        values($propoliza[id_propiedad_poliza],$eliddeltitu,0,'$fecha','$hora',$propoliza[organo],
                                           '$propoliza[monto]','$propoliza[monto]');");
      ...
      $haycliente=num_filas($repbushiclien);
        if($haycliente>=1){
            ...
            $idclienhijo=$datrehiclien[id_cliente];//ya tenemos el id_cliente del beneficiario!!
               ...
               $bushijclien=("select clientes.id_cliente from clientes where clientes.cedula='$cedben' and
                                       clientes.nombres='$nombrben';");
    ...
    $guardorecibocontrato=("insert into tbl_recibo_contrato(id_contrato_ente,num_recibo_prima,fecha_ini_vigencia,
                                             ...
                                             values($elcontratoid,'$numcontraprima','$iniciocotrato','$findecotrato','$fecha','$hora',$idcomisionado);");
...
coberturas_t_b.id_titular,coberturas_t_b.id_beneficiario,propiedades_poliza.id_poliza order by id_beneficiario;");
    ...
    while($loscobtura=asignar_a($reptodocobert,NULL,PGSQL_ASSOC)){
        ...
        //echo "-----------------------1-$elcienteides<br>";
        //guardamos el cliente en la tabla entes

        $guardente=("insert into entes(nombre,telefonos,direccion,rif,email_contacto,fecha_creado,hora_creado,
                                                        id_ciudad,fecha_inicio_contrato,fecha_renovacion_contrato,
                                                        id_sucursal,id_tipo_ente,fecha_inicio_contratob,fecha_renovacion_contratob,
                                                        es_med_pre) values('$datcliente[nombres] $datcliente[apellidos]',
                                                        '$datcliente[telefono_hab]','$datcliente[direccion_hab]','$datcliente[cedula]',
                                                        '$datcliente[email]','$fecha','$hora',$ciudadtitu,'$iniciocotrato','$findecotrato',
                                                        $lasucursal,7,'$iniciocotrato','$findecotrato',1);");
        //echo "<br> $guardente";
     $repguaente=ejecutar($guardente);
      //buscamos el id_ente que se genera
     ...
     //echo "-----------------------<br>3-$guardartitu<br>";                            
       ...
       //echo "-----------------------<br>3-$guardartitu<br>";                       
        ...
        $repguartitu=ejecutar($guardartitu);
       //una vez registrado el titular buscamos cual es el id_titular
      $buscidtitu=("select titulares.id_titular from titulares 
                       where titulares.id_cliente=$elcienteides and fecha_creado='$fecha' and hora_creado='$hora';");    
      $repidtitu=ejecutar($buscidtitu);
      $datadidtitu=assoc_a($repidtitu);
      $eliddeltitu=$datadidtitu[id_titular];//--------------------->ya tenemos el id_titular
      //echo "-----------------------<br>4-$eliddeltitu<br>"; 
       //luego de tener el id_titular lo guardamo en la tabla titulares_subdivisiones y lo guardamos con la
    //id_subdivision=86 que es PARTICULAR
    $guardtitusubdvi=("insert into titulares_subdivisiones(id_titular,id_subdivision) values($eliddeltitu,$subdititu);");
    $repgustdbudi=ejecutar($guardtitusubdvi);
     //echo "-----------------------<br>4-$guardtitusubdvi<br>";
    //guardamos el titular en la tabla estados_t_b
    $guardtituestatb=("insert into estados_t_b(id_estado_cliente,id_titular,id_beneficiario,fecha_creado,hora_creado) 
                                              values($titulaestado,$eliddeltitu,0,'$fecha','$hora');");
     //echo "-----------------------<br>5-$guardtituestatb<br>"  ;                                       
...
coberturas_t_b.id_titular,coberturas_t_b.id_beneficiario,propiedades_poliza.id_poliza order by id_beneficiario;");
  ...
  }//Fin del else donde el cliente existe
