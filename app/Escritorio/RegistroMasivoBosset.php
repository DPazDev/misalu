...
$filename = "BASE DE DATOS BOSSET.xls";
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
///EL ENTE
	   $entclien='79';//ID ENTE BOSSET
		$poliza='228';//PLAN CLINI LABORAL ENTRY
		$fecha=date("Y-m-d");
		$hora=date("H:i:s");
///contadores
		$clientesnuevos=0;
...
$NunPolizaEnte=num_filas($consultaPolizEnte);
 ...
 for ($i = 5; $i <= $data->sheets[0]['numRows']; $i++) {
   ...
   echo "<h1><br>$cedclien</h1>";
          //validar cedula sin puntos ni letras
              $cedclienSinFormato=$cedclien;
              $cedclienSinGion= str_replace('-', '', "$cedclienSinFormato");
              $cedclienSinV= str_replace('V', '', "$cedclienSinGion");
              $cedclienSinv= str_replace('v', '', "$cedclienSinV");
              $cedclienSinPunto= str_replace('.', '', "$cedclienSinv");
              $cedclien=$cedclienSinPunto;
              echo "<h1><br>$cedclien</h1>";
          //fin Formato ci
...
if($cuantosexiste>=1) {
		...
		//Actualizar Cliente
		$actdatoscliente=("update clientes set sexo='$genercli', direccion_hab='$direcclient',telefono_hab='$clitf1',telefono_otro='$clitf2',comentarios=upper('$comentclien') where clientes.id_cliente='$elidcliente';");
...
//echo "<b><h1> INSERTAR NO SE ENCONTRO EL ESTADO</h1>";
			//no existe en estados tb
					...
					echo "<h4> actualizar estatus $clienestatus </h4>";
