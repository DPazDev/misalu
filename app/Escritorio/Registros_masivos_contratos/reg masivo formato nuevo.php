...
$archivo = 'colegio_cardenal_quintero.xlsx';
...
$rifEnteContratante = "J090262636"; // CAJA DE AHORRO Y PREST DE LOS TRAB. AFILIADOS AL SINDICATO DE HOSP. Y CLINICAS MERIDA
$idSubdivision = 273; // Caja de Ahorro del Sindicato de Clínicas y Hospitales Mérida
...
foreach ($filas as $fila) {
    ...
    if ($tipoRegistro === 'TITULAR') {
        ...
        $registroActual["comentarios"] = trim($registroActual["comentarios"]);
        ...
        if (!empty($titulares)) {
            ...
            $beneficiario["comentarios"] = trim($beneficiario["comentarios"]);
...
try {
    foreach ($titulares as &$titular) {
        ...
        if (!$filaClienteTitular) {
            ...
            // La edad y la fecha de nacimiento siempre se actualizan
            $actualizarData = array(
                "edad = '{$titular['edad']}'",
                "fecha_nacimiento = '{$titular['fechaNacimiento']}'"
            );


            // Actualizar telefono, email o comentarios si no están vacíos
            if (!empty($titular['telefono'])) {
                $actualizarData[] = "telefono_hab = '{$titular['telefono']}'";
            }

            if (!empty($titular['email'])) {
                $actualizarData[] = "email = '{$titular['email']}'";
            }

            if (!empty($titular['comentarios'])) {
                $actualizarData[] = "comentarios = '{$titular['comentarios']}'";
            }

            $sqlActualizarClienteTitular = "UPDATE clientes SET " . implode(", ", $actualizarData) . 
        ...
        foreach ($titular["beneficiarios"] as &$beneficiario) {
            ...
            if (!$filaClienteBeneficiario) {
                ...
                // Nombres, apellidos, edad y fecha de nacimiento siempre se actualizan en caso de que ya exista el beneficiario
                $actualizarData = array(
                ...
                // Actualizar telefono y email si no están vacíos
                if (!empty($beneficiario['telefono'])) {
                    $actualizarData[] = "telefono_hab = '{$beneficiario['telefono']}'";
                }

                if (!empty($beneficiario['email'])) {
                    $actualizarData[] = "email = '{$beneficiario['email']}'";
                }

                if (!empty($beneficiario['comentarios'])) {
                    $actualizarData[] = "comentarios = '{$beneficiario['comentarios']}'";
                }

                $sqlActualizarClienteBeneficiario = "UPDATE clientes SET " . implode(", ", $actualizarData) . 
