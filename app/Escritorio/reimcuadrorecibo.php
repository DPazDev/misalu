...
clientes.cedula,clientes.id_cliente,clientes.nombres,clientes.apellidos,
  titulares.id_titular,tbl_contratos_entes.numero_contrato,tbl_contratos_entes.id_ente,
   tbl_recibo_contrato.num_recibo_prima;");
  $repuestdacontr=ejecutar($busdatoscontr);
  $datcontra=assoc_a($repuestdacontr);
$idcontrato=$datcontra['numero_contrato'];
$idrecibo=$datcontra['num_recibo_prima'];
$idtitular=$datcontra['id_titular'];
$lacotizacion=$_REQUEST['lacotiza'];
$nomcompleto="$datcontra[nombres] $datcontra[apellidos]";
...
$verpolizas=("select polizas.id_poliza,polizas.nombre_poliza,titulares_polizas.id_titular 
...
<table   class="tabla_citas"  cellpadding=0 cellspacing=0 border=1>
        ...
        <tr>
         ...
         <tr>
          ...
          <?$filas=1; 
            while($laspoliza=asignar_a($repvepoliza,NULL,PGSQL_ASSOC)){
                ...
                $propiedpoliza=("select propiedades_poliza.cualidad,propiedades_poliza.descripcion,propiedades_poliza.monto 
                                            ...
                                            where id_poliza=$idpoliza order by propiedades_poliza.cualidad");
                $repdapoliza=ejecutar($propiedpoliza);
                ?>        
         </tr>
                <td colspan=12 align="center" class="titulo3"><strong>Poliza: <? echo $laspoliza[nombre_poliza]?></strong></td>
         </tr>
            <?if($filas==1){?>
                <tr>
         ...
         <?  
           ...
           $queplan=("select polizas.nombre_poliza,polizas.deducible from polizas where polizas.id_poliza=$idpoliza"); 
           $requeplan=ejecutar($queplan);
           $datqueplan=assoc_a($requeplan);
          $haydeduci=$datqueplan['deducible'];
         ?>
            <?php if($haydeduci>0){?>
         <tr>
                  
                  <td rowspan=1 colspan=6>Deducible</td>
                  <td rowspan=1 colspan=2><?echo $haydeduci?></td>
         </tr>
         <?php }?>
         <tr>
        ...
        <tr>
         ...
         <tr>
              ...
              <td colspan=2 class="tdtituloss"><label style="font-size: 8pt"><?echo calcular_edad($losbeni[fecha_nacimiento]);?></label></td>     
...
echo "00000$refer"?></strong></label></td>
        ...
        <tr>
                  <td colspan=12 class="tdtituloss"><label style="font-size: 7pt"><strong>Yo <?echo $nomcompleto?> 
                    ...
                    lo establecido en el articulo 40 de la Providencia Administrativa N° 514 de fecha 18 de febrero de 2011 emanada por la Superintendencia de la Actividad Aseguradora, 
