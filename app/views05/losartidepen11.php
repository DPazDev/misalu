<?php
////ANULAR EL PEDIDO DE LA DEPENDECIA
include ("../../lib/jfunciones.php");
sesion();
$idpedido=$_POST['elidpedido'];
$DataControl=$_POST['DataControl'];
$bloquears=$_POST['bloquear'];
if($bloquears!=1){
$buscaridpeentre=("select tbl_ordenes_pedidos.id_orden_entrega from tbl_ordenes_pedidos where tbl_ordenes_pedidos.id_orden_pedido=$idpedido");
$repbuscaridentre=ejecutar($buscaridpeentre);
$cuantos=num_filas($repbuscaridentre);
  if($cuantos>0){
  $dataentrega=assoc_a($repbuscaridentre);
  $identrega=$dataentrega['id_orden_entrega'];
  }else{$identrega=$idpedido;}
}else{
$identrega=$idpedido;
}
...
$buscarpedido=("select
       ...
       where
           tbl_insumos_ordenes_pedidos.id_orden_pedido=$identrega and
...
$cometpedido=("select tbl_ordenes_entregas.comentario,tbl_ordenes_entregas.id_orden_pedido from tbl_ordenes_entregas where id_orden_entrega=$identrega");
...
<br>
   ...
   <td><label title="Anular pedido" class="boton" style="cursor:pointer" onclick="Apedido2(<?echo $idpedido?>)" >Anular Pedido</label>
   </td>
