...
$fecha=date("Y-m-d");
$hora=date("H:i:s");
$codigo=$_REQUEST['laopcion'];
$busqueda=$_REQUEST['pabuscar'];
$tipoinsumo=$REQUEST['insumoid'];
if($tipoinsumo==0){
  $querytipinsumo="";
}else{
...
tbl_insumos_almacen.id_dependencia,tbl_dependencias.dependencia,tbl_insumos_almacen.cantidad  from 
...
if($cuantos<=0){
    echo"<table class=\"tabla_citas\"  cellpadding=0 cellspacing=0> 
     <tr> 
         <td colspan=3 class=\"datos_cliente3\">No hay informaci&oacute;n!!!!</td>
	</tr>	 
 </table>	";
   }else{ 
?>
...
<table class="tabla_citas"  cellpadding=0 cellspacing=0 border=1>    
             <tr>
                 <th class="datos_cliente3">C&oacute;digo.</th>
                 <th class="datos_cliente3">Art&iacute;culo.</th>  
                 <th class="datos_cliente3">Laboratorio.</th>
               <?if(($busqueda=='a') or ($todas=='si')){
                   echo "<th class=\"datos_cliente3\">Dependencia.</th>";
                   }?>
		 <th class="datos_cliente3">Tipo de art&iacute;culo.</th> 	
				...
				?>
			    <tr>
				   <td class="cantidades"><?echo "$articulos[id_dependencia]-$articulos[id_insumo]-$articulos[id_tipo_insumo]";?></td>
				   <td class="cantidades"><?echo $articulos['insumo'];?></td>
                                   <td class="cantidades"><?echo $articulos['laboratorio'];?></td> 				  
                                    <?if(($busqueda=='a')or($todas=='si')){
                                     echo "<td class=\"cantidades\"> $articulos[dependencia]</td>";
                                    }?>
                                   <td class="cantidades"><?echo $articulos['tipo_insumo'];?></td>      				   <td class="tdcampos"><?echo $articulos['cantidad'];?></td> 
				</tr>
			<?}
        }    ?>		
