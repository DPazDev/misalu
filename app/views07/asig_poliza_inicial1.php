<?
/*  Nombre del Archivo: asig_poliza_inial1.php
   Descripción: Busqueda en base de datos para Asignar Póliza a Cliente ya que no le asignaron poliza al ser Registrado
*/
...
<?php if($f_cliente[id_cliente]==$f_ti[id_cliente]){
	...
	$q_polizat=("select titulares.id_titular,titulares.id_ente,entes.nombre from titulares,clientes,entes where 
	...
	titulares_polizas.id_poliza=polizas.id_poliza and
 	titulares_polizas.id_poliza=propiedades_poliza.id_poliza and 
	entes.id_ente=titulares.id_ente
	...
	<tr>
	       ...
	       <td class="tdcampos"  colspan="1"><select id="poliza_t" name="poliza" class="campos"  style="width: 210px;" onchange="bus_poliza(4)">
                                     <option value= ""> Seleccione el Ente</option>
<?php $id_beneficiario='0';
				       while($f_polizat=asignar_a($r_polizat,NULL,PGSQL_ASSOC)){?>
				     <option value="<?php echo "$f_polizat[id_ente]-$f_polizat[id_titular]-$id_beneficiario"?>"> <?php echo "$f_polizat[nombre]"?></option>
<?}?>			
		</select>
...
$poliza_b="";
	$q_polizab=("select beneficiarios.id_titular,beneficiarios.id_beneficiario,titulares.id_ente,entes.nombre from titulares,clientes,entes,beneficiarios where 
	clientes.cedula='$ci' and
...
<tr>
	       <td class="tdtitulos" colspan="1">&nbsp; Seleccione el Ente:</td>
	       <td class="tdcampos"  colspan="1"><select id="poliza_b" name="poliza" class="campos"  style="width: 210px;" onchange="bus_poliza(4)" >
                                     <option value= ""> Seleccione el Ente</option>
				     <?php  while($f_polizab=asignar_a($r_polizab,NULL,PGSQL_ASSOC)){?>
				     <option value="<?php echo "$f_polizab[id_ente]-$f_polizab[id_titular]-$f_polizab[id_beneficiario]"?>">  <?php echo "$f_polizab[nombre]"?></option>
<?}?>			
		</select>
...
<td  class="tdcamposcc"  colspan="4"><a href="#" OnClick="guard_poliza_inicial();" class="boton">Guardar</a> <a href="#" OnClick="ir_principal();" class="boton">Salir</a></td>
</tr>
...
<div id="asig_poliza_inicial1"></div>
