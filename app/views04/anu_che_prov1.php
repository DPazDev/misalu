...
$q_facturas_procesos="select 
                                                * 
                                        from 
                                                facturas_procesos 
                                        where 
                                                facturas_procesos.numero_cheque='$numerocheque' and 
                                                facturas_procesos.id_banco=$id_banco";
...
$numero_cheque=$f_facturas_procesos[numero_cheque];
$motivoante = $f_facturas_procesos[motivo];
$codigo1=$admin . $f_facturas_procesos[codigo] . $fecha;
...
values ($f_facturas_procesos[id_proceso],
            '$f_facturas_procesos[fecha_creado]',
            '$hora',
            $admin,
...
$mod_fpro="update facturas_procesos set id_banco=9,id_banco_anulado=$id_banco,motivo='$motivo',compro_retiva_seniat='0',corre_retiva_seniat=0,corre_compr_islr='0',corre_ret_islr=0 
where facturas_procesos.numero_cheque='$numerocheque' and facturas_procesos.id_banco='$id_banco'";
$fmod_fpro=ejecutar($mod_fpro);

/* **** Se registra lo que hizo el usuario**** */
$log="Se Anulo el Cheque   numero $numerocheque y id_banco  $id_banco motivo $motivo y motivo anterior $motivoante";
logs($log,$ip,$admin);
