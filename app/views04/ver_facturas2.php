...
$q_factura="select
					...
					from
							...
							admin
					where
          CAST (tbl_facturas.numero_factura as integer)='$factura' and
...
$temporalplanilla ="select procesos.nu_planilla from tbl_procesos_claves, procesos  where tbl_procesos_claves.id_factura = $f_factura[id_factura] and
tbl_procesos_claves.id_proceso=procesos.id_proceso limit 1";
$reptemporalplanilla=ejecutar($temporalplanilla);
$dataplanillas=assoc_a($reptemporalplanilla);
$numdelaplanilla=$dataplanillas[nu_planilla];


if(!empty ($numdelaplanilla) || $numdelaplanilla!=0){
$totalmtno=("select sum(cast(gastos_t_b.monto_aceptado as double precision)) from gastos_t_b,procesos where procesos.nu_planilla='$numdelaplanilla' and
...
<td colspan=2 class="tdcamposr"><input type="hidden" id="monto" name="monto" class="campos" size=20 value="<?php echo montos_print($total)?>"><?php echo montos_print($total)?></td>
</tr>
<tr>
<td  class="tdtitulos"></td>
<td  class="tdcamposr">Monto no Aprobado.</td>
<td colspan=2 class="tdcamposr"><?php echo montos_print($montotdecno)?></td>
</tr>
<tr>
<td  class="tdtitulos"></td>
<td  class="tdcamposr">Total BF.</td>
<td colspan=2 class="tdcamposr"><input type="hidden" id="monto" name="monto" class="campos" size=20 value="<?php echo montos_print(abs($total - $montotdecno))?>"><?php echo montos_print(abs($total - $montotdecno))?></td>
<input type="hidden" id=monto_fac name="monto_fac" value="<?php echo $total - $montotdecno ?>">
</tr>
<tr>
<td  class="tdtitulos"></td>
<td  class="tdcamposr">Deducible</td>
...
<?php } ?>
<tr><td colspan=4  class="titulo_seccion"> Formatos de la Factura
...
<?php $url="'views04/ifactura2USD.php?factura=$f_factura[numero_factura]&serie=$f_factura[id_serie]','700','500'";
			?> <a href="javascript: imprimir(<?php echo $url; ?>);" class="boton" title="Factura con los Datos del Cliente"> Formato USD  </a>

<?php $url="'views04/ifactura_iva.php?factura=$f_factura[numero_factura]&serie=$f_factura[id_serie]','700','500'";
...
$q_notacre="select * from tbl_nota_factura where tbl_nota_factura.id_factura=$f_factura[id_factura]";
...
if ($num_filasn>0)
{
    ...
    ?>
    <tr>
      <td colspan=4  class="titulo_seccion Nun-Negativo-nota">Notas Contable exitentes para factura <?php echo $f_factura['numero_factura']." SERIE ".$f_factura['nomenclatura'];?></td>
    </tr>
    <tr>
      <td colspan=4  class="tdcamposr">
        <table  class="tabla colortable" style="which:100%;min-width:100%;" border=0 cellpadding=0 cellspacing=0>
          <?php if($NunDebito>0){?>
            <tr>
              <th colspan=4 class="tdcamposc">NOTAS DE DÉBITO</th>
            </tr>
          <?php } ?>
          <tr>
            <td class="tdtitulos" width="25%">Tipo de Nota</td>
    ...
    <?php
        $totalNotaC=0;
        $totalNotaD=0;
          while($notasExitentes = asignar_a($r_notacre)){
              $numNota=$notasExitentes['num_nota'];
              $TipoNota=$notasExitentes['tipo_nota'];
              $numControlNota=$notasExitentes['numcontrolnota'];
              $FechaEmiNota=$notasExitentes['fecha_emision'];
              $MontoNota=$notasExitentes['monto_nota'];

              if($TipoNota==1){
                ///CREDITO
                $ClassNumb="Nun-Negativo-nota";
                $TipoNotaFactura='CRÉDITO';
                $totalNotaC=$totalNotaC+$MontoNota;
              }else{
                $ClassNumb="Nun-Positivo-nota";
                //DEBITO
                $TipoNotaFactura='DÉBITO';
                $totalNotaD=$totalNotaD+$MontoNota;
              }
            ...
            ?>
              <tr>
                <td  class="tdcampos"><?php echo" $TipoNotaFactura  $TipoNota";?></td>
                ...
                <td  class="tdcampos"><?php echo $numControlNota;?></td>
                <td  class="tdcampos"><?php echo $FechaEmiNota;?></td>
                <td  class="tdcampos <?php echo $ClassNumb;?>"><?php echo $MontoNota;?></td>
              </tr>
              <?php
            } ?>
              <tr>
                ...
                <td  class="tdcamposc Nun-Positivo-nota">(+)<?php echo $totalNotaD;?></td>
              </tr>
              <tr>
                <td colspan=3 class="tdcamposc"></td>
                <td  class="tdcamposc" style="text-align:right;">Total Credito:</td>
                <td  class="tdcamposc Nun-Negativo-nota">(-)<?php echo $totalNotaC;?></td>
              </tr>
              <?php



        $facturaTotal=($total+$totalNotaD)-$totalNotaC;
      ?>
      <tr>
        <td colspan=3 class="tdcamposc">
          (Factura Total + Total Debito)- Total Credito<br>
          <?php echo "( $total + $totalNotaD ) - $totalNotaC";?>
        </td>
...
?>
<tr>
  <td colspan=4  class="titulo_seccion">Datos de la Factura</td>
</tr>
<tr>
...
<a href="#"
OnClick="mod_edo_fact();" title="Modifica el Estado de La Factura" class="boton">Modificar Estado</a>
</td>
</tr>
...
<tr>
  ...
  else{   //***************************************************** MODIFICACION ING PATRICIA 07-10-2022
       ?>
      	     <tr>
...
<tr><td>
	<?php if($enablemr==""){ ?>
  <a href="views04/mod_datos_pago.php?factura=<?echo $f_factura[numero_factura]?>" title="Modificar datos del pago" <?php echo "$enablemr"; ?> class="boton" onclick="Modalbox.show(this.href, {title: this.title, width:700,height:300, overlayClose: false}); return false;">Modificar</a>
  </td></tr>
<?php }?>
...
<td class="tdcampos"></td>
		...
		<tr>		<td colspan=4 class="titulo_seccion">Datos Para Modificar Facturas</td>	</tr>
<tr>
...
<img src="../public/images/calendar.gif" class="cp_img" alt="Seleccione la Fecha"></a>
	...
	<tr>
		<td class="tdtitulos">Proceso</td>
		<td class="tdcampos"><input class="campos" type="text" id="proceso" name="proceso" maxlength=128 size=20 value=""   onkeypress="return event.keyCode!=13"> o</td>

		</tr>
		<tr>
		<td class="tdtitulos">Clave</td>
		<td class="tdcampos"><input class="campos" type="text" id="clave" name="clave" maxlength=128 size=20 value=""   onkeypress="return event.keyCode!=13"> o</td>

		</tr>
		<tr>
		<td class="tdtitulos">Presupuesto o Planilla</td>
		<td class="tdcampos"><input class="campos" type="text" id="planilla" name="planilla" maxlength=128 size=20 value=""   onkeypress="return event.keyCode!=13"> o</td>

		</tr>
        </tr>
        <td class="tdtitulos">Cuadro Recibo de Prima</td>
		<td class="tdcampos"><input class="campos" type="text" id="cua_rec_prim" name="cua_rec_prim" maxlength=128 size=20 value=""   onkeypress="return event.keyCode!=13"> o</td>
		...
		<td>
		 <select  style="width: 300px;" id="entes" name="entes" class="campos">
                ...
                <?php
				$q_entes=("select * from entes  order by nombre");
...
<tr>
 ...
 <tr>
                <td  colspan=4  class="tdtitulos">Seleccione la Sucursal si Selecciona un Ente
 ...
 <tr>
                <td  colspan=4   class="tdtitulos">Seleccione el Servicio si Selecciona un Ente
                ...
                <input class="campos" type="hidden" name="vacio" maxlength=128 size=20 value=""><a href="#"
OnClick="buscar_procesosmf();" class="boton">Buscar</a><a href="#" OnClick="ir_principal();" class="boton">Salir</a></td>
