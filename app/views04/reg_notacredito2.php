...
<td  colspan=4  class="titulo_seccion">La factura No Existe</td>
	...
	else
	{
		...
		$q_notaCredito="select * from tbl_nota_factura,tbl_facturas where tbl_facturas.id_factura=tbl_nota_factura.id_factura and tbl_facturas.id_serie=$f_series[id_serie] and tbl_nota_factura.tipo_nota=1 order by tbl_nota_factura.id_nota_factura desc limit 1;";
		...
		if(num_filas($r_notacredito)==0){
			$no_notacredito="0001";
			$ultimoNumControlNota=0;
		}else{
			$f_nota=asignar_a($r_notacredito);
			$no_notacredito=(int)$f_nota[num_nota];
			$ultimoNumControlNota=(int) $ultCrtN['numcontrolnota'];
			if($no_notacredito<=10){
				$no_notacredito++;
				if($no_notacredito==10)	$no_notacredito="00$no_notacredito";
				else			$no_notacredito="000$no_notacredito";
			}else if($no_notacredito>10 && $no_notacredito<=100){
		                $no_notacredito++;
				if($no_notacredito==100)    $no_notacredito="0$no_notacredito";
				else                    $no_notacredito="00$no_notacredito";
			}else if($no_notacredito>100 && $no_notacredito<=1000){
				$no_notacredito++;
				if($no_notacredito==1000)     $no_notacredito="$no_notacredito";
				else                      $no_notacredito="0$no_notacredito";
			}else{
				$no_notacredito++;
			}
		}
...
///GASTOS DE FACTURA
		...
		////si es obligatorio mostrar el Ente
			...
			<td colspan=4>
				<table class="tabla colortable" style="which:100%;min-width:100%;" border=0 cellpadding=0 cellspacing=0 >
						<?php
							...
							pg_result_seek($r_procesos,0);//inicio el apuntador de DB en 0
							...
							<tr>
								...
								<th class="tdtitulos" colspan=1 >Bs.S.</th>
							</tr>
						...
						<tr>
							...
							<th  class="tdcamposr">Total BF.</th>
							<th colspan=1 class="tdcamposr"><input type="text" readonly id="monto" class="campos" size="10" name="monto" value="<?php echo $total; ?>"></th>
						</tr>
			</table>
		</td>
		</tr>
...
//////////////////////CONSULTAR SI LA FACTURA TIENE NOTAS DE DEBITO Y CREDITO////////////////////////
		...
		?>
		<tr>
			<td colspan=4  class="verificar_fecha">Notas exitentes para factura <?php echo $f_factura['numero_factura']." SERIE ".$f_factura['nomenclatura'];?></td>
		</tr>
		<tr>
		...
		$totalDebito=0;
			...
			<tr>
				...
				<td  class="tdcamposc" style="text-align:right;">Total Deuda Factura:</td>
				<td  class="tdcamposc">
						<?php echo $facturaTotal;?>
	...
	<?php }	?>
	<tr>
		<td colspan="4"><input class="campos" size="8" type="hidden" id="TotalFacturado" name="TotalFacturado" value="<?php echo $facturaTotal?>"></td>
	<tr>
<!-- 	///////////// FIN CONSULTAR SI LA FACTURA TIENE NOTAS DE DEBITO Y CREDITO//////////////////////// -->


<!-- ////////////// DATOS PARA CREAR LA NOTA DE Credito /////////////////-->
		<tr><td colspan=4  class="titulo_seccion"> Datos de la Factura</td></tr>
		<tr>
			<td  class="tdtitulos">No. de Factura</td>
			<td  class="tdcampos">
					<input type="hidden" id="id_factura" name="id_factura" value="<?php echo $f_factura[id_factura]; ?>">
					<input type="hidden" id="num_notacredito" name="num_notacredito" value="<?php echo $no_notacredito; ?>"><?php echo $no_notacredito; ?>
			</td>
			<td class="tdtitulos">Serie</td>
			<td class="tdcampos">
				<input type="hidden" id="serie" name="serie" value="<?php echo $f_factura[id_serie]; ?>"><?php echo $f_factura[nomenclatura]; ?>
			</td>
		</tr>
		<tr>
			<td  class="tdtitulos">No. de Control Factura</td>
			<td  class="tdcampos">
				Ultimo numero de control serie:<span class="tdcamposr"><?php echo $ultimoNumControl;?></span><br>
				<input class="campos" size="8" type="hidden" id="UltimoControlFactura" name="UltimoControlFactura" value="<?php echo $ultimoNumControl?>">
				<input class="campos" size="10" type="text" id="controlfactura" name="controlfactura" value="">
			</td>
			<td  class="tdtitulos">* Fecha Emision   </td>
			<td class="tdcampos">
		 		<input  type="text" size="10" id="dateField3" name="fechar" class="campos" maxlength="10" value="<?php echo $fecha_creado?>" >
	   	</td>
		</tr>
		<tr>
			<td class="tdtitulos"  colspan=1 align="left">Clasifica de Nota</td>
			<td class="titulos" colspan=1 align="left">
				<select id="estado_cre" name="estado_cre" class="campos" style="width: 200px;" onchange="std=$F('estado_cre'); if(std==1){$(MontoTotal).value=$F('monto');}else{$(MontoTotal).value=0;}" >
					<option value="1" >Por Devolucion</option>
					<option value="2" >Por Descuento</option>
				</select>
			</td>
		</tr>
		<tr>
			<td colspan=3 class="tdtitulos">CONCEPTO</td>
		  <td colspan=1 class="tdtitulos">Monto Nota de Crédito</td>
		</tr>
		<tr>
			<td colspan=3 class="tdcampos"><textarea id="concepto" name="concepto" maxlength='250' cols=81 rows=4  style="text-transform: uppercase;" class="campos"><?php echo $f_factura[concepto]?></textarea></td>
		  <td colspan=1 class="tdcampos"><input type="text"  id="MontoTotal" class="campos" size="8" name="MontoTotal" value="<?php echo $total; ?>""></td>
		</tr>
		<tr>
			<td class="tdtitulos"></td>
			<td class="tdtitulos"></td>
			<td class="tdtitulos"></td>
			<td class="tdcampos"><input class="campos" type="hidden" name="vacio" maxlength=128 size=20 value=""><a href="#" OnClick="guardar_notacredito();" class="boton" title="Guardar La Nota de Credito">Guardar</a></td>
 		</tr>
</table>
