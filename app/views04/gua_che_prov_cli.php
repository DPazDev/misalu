...
$procesos=0;
	...
	if ($tipo_documento=='1'){
			$honorarios_medicos= ($honorarios_medicos * (-1));
			$gastos_clinicos= ($gastos_clinicos * (-1));
			$ret= ($ret * (-1));
			$iva= ($iva * (-1));
			$retiva= ($retiva * (-1));
...
(id_proceso,fecha_creado,hora_creado,id_admin,id_servicio,codigo,id_proveedor,numero_cheque,comprobante,
...
values
('$procesos','$fechacreado','$hora','$admin','$idservicios','$codigo','$proveedor','$numcheque','$no_comprobante',
'$honorarios_medicos','$ret','0','$iva','$gastos_clinicos','$banco','$rif','$prov','$factura','$no_factura1',
'0','$no_comp_ret_iva1','$no_comp_ret_iva','$retiva','$controlfactura','$fecha_emision',
'$no_comp_ret_islr1','$no_comp_ret_islr','$anombrede','$cedularif','$motivo','$tipocuenta','$ret_indi','$tipo_documento','$id_variable','$fac_afectada');
";
}
else
{
...
///ACTUALIZAR PAGO SI EXISTE LA REFENCIA BANCO Y NUMERO 12-07-2023
if($banco=='13' || $banco=='15'){$ref=0;}//excluir Comprobantes y Cheque por Generar

if($ref==1){
		if(!empty($codigo) && $codigo>0 ){
			echo$mod_fproceso="update facturas_procesos
								set
												numero_cheque='$numcheque',
												id_banco=$banco,
												motivo='$motivo',
												id_tipo_cuenta='$tipocuenta',
												fecha_imp_che='$fechacreado',
		...
		//SE REGISTRA LAS ACIONES
		$log="Actulizacion de $descripcion  codigo numero $codigo";
		logs($log,$ip,$admin);
}
...
<tr>
		...
		if ($f_admin[id_tipo_admin==11] || $f_admin[id_tipo_admin==7]){
			...
			$url="'views04/icheque_prov_islr.php?codigo=$codigo&fechaemision=$fechacreado&tipocuenta=$tipocuenta&numcheque=$numcheque&motivo=$motivo&banco=$banco&nombreprov=$nombreprov&cedula=$rif&comproretiva=$no_comp_ret_iva1&prov=$prov&id_proveedor=$proveedor&personaprov=$personaprov&mod=1'";
