...
$eli_nomina="delete from nomina where  nomina.id_nomina='$cid_nomina'";
$feli_nomina=ejecutar($eli_nomina);
$eli_nominatb="delete from nomina_tb where  nomina_tb.id_nomina='$cid_nomina'";
$feli_nominatb=ejecutar($eli_nominatb);

/* **** Se registra lo que hizo el usuario**** */
$log="Se elimino la nomina  $cid_nomina por $ccomentario ";
logs($log,$ip,$id_admin);
          ...
          <? while($nomida=asignar_a($renominact,NULL,PGSQL_ASSOC)){
             ...
             $forpago=$nomida['formpago'];?>
               <tr>  
                    ...
                    <td><label title="Calcular Nomina" id="enteguardado1" class="boton" style="cursor:pointer" onclick="RepNomina('<?echo $nomida[id_nomina]?>','<?echo $nomente?>','<?echo $nomida[formpago]?>','<?echo $nomida[codigo]?>')" >N</label></td>  
