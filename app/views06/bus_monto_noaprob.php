...
if ($forma_pago==4){
    ...
    SUM(CASE WHEN tbl_facturas.descuento > 0 THEN tbl_procesos_claves.monto * (1 - tbl_facturas.descuento / 100) ELSE tbl_procesos_claves.monto END), 
    SUM(tbl_procesos_claves.fac_deducible) AS sum_deducible
FROM 
    tbl_facturas
INNER JOIN 
    tbl_procesos_claves ON tbl_facturas.id_factura = tbl_procesos_claves.id_factura
INNER JOIN 
    procesos ON tbl_procesos_claves.id_proceso = procesos.id_proceso
INNER JOIN 
    titulares ON procesos.id_titular = titulares.id_titular
INNER JOIN 
    entes ON titulares.id_ente = entes.id_ente
INNER JOIN 
    tbl_tipos_entes ON entes.id_tipo_ente = tbl_tipos_entes.id_tipo_ente
INNER JOIN 
    tbl_series ON tbl_facturas.id_serie = tbl_series.id_serie
WHERE 
    tbl_facturas.id_factura=tbl_procesos_claves.id_factura
    ...
    AND tbl_procesos_claves.id_proceso=procesos.id_proceso
    AND procesos.id_titular=titulares.id_titular
    AND titulares.id_ente=entes.id_ente 
    $elente
    AND entes.id_tipo_ente=tbl_tipos_entes.id_tipo_ente
    AND
    $eltipo_ente 
    $tipo_pago  
    $andnumcheque
