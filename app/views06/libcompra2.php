...
facturas_procesos.factura,facturas_procesos.no_control_fact,
...
sum(cast(facturas_procesos.iva as double precision)) as iva,
sum(cast(facturas_procesos.retencion as double precision)) as retencion
from 
...
$repdelasfacturas=ejecutar($buscolasfacturas);
  ...
  while($ellibroc=asignar_a($repdelasfacturas,NULL,PGSQL_ASSOC)){ 
	  ...
	  $eliva=$ellibroc['iva'];
          ...
          if(($montoconretencion==0)&& ($eliva>0)){
              $montotalexento = 0;  
            }else{
                 if(($montoconretencion>0) && ($eliva>0)){
