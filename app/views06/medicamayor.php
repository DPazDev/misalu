...
(clinicas_proveedores.id_clinica_proveedor=238 or clinicas_proveedores.id_clinica_proveedor=180) order by clinicas_proveedores.nombre;");
$repfarmacia=ejecutar($farmacias);

$servicio=("select servicios.id_servicio,servicios.servicio from servicios where (servicios.id_servicio=9 or servicios.id_servicio=6) order by servicio;");
$repservicio=ejecutar($servicio);

$q_insumo=("select tbl_tipos_insumos.* from tbl_tipos_insumos where (tbl_tipos_insumos.id_tipo_insumo=1 or tbl_tipos_insumos.id_tipo_insumo=2) order by tbl_tipos_insumos.tipo_insumo ;");
$r_insumo=ejecutar($q_insumo);
...
<table class="tabla_cabecera3"  cellpadding=0 cellspacing=0>
     <tr>
         <td colspan=4 class="titulo_seccion">Control de Med&iacute;camentos con Mayor Salida</td>
     </tr>
</table>
 ...
 <table class="tabla_citas"  cellpadding=0 cellspacing=0> 
     ...
     <tr>
        <td colspan=2 class="tdtitulos">* Seleccione el ente:</td>
         <td class="tdcampos"> <select id="ente" class="campos"  style="width: 150px;">
			        <option value=""></option>
					 <option value="1@Todos">Todos</option> 
           <?php  while($elente=asignar_a($repentes,NULL,PGSQL_ASSOC)){?>
						<option   value="<?php echo "$elente[id_ente]@$elente[nombre]"?>"> <?php echo "$elente[nombre]"?></option>
			      <?php
			             }
		              ?>
		              </select></td> 
        ...
        <td class="tdcampos"> <select id="estaproce" class="campos"  style="width: 150px;">
           ...
           <?php  while($estadproce=asignar_a($repestadospro,NULL,PGSQL_ASSOC)){?>
						<option   value="<?php echo "$estadproce[id_estado_proceso]@$estadproce[estado_proceso]"?>"> <?php echo "$estadproce[estado_proceso]"?></option>
			      <?php
			             }
		              ?>
		              </select></td> 
	...
	<tr><td>&nbsp;</td></tr>
     ...
     <tr> 
	    <td colspan=2 class="tdtitulos">* Seleccione tipo de Insumo:</td>
         <td class="tdcampos"> <select id="insumo" class="campos"  style="width: 150px;" >
			        <option value=""></option>
           <?php  while($f_insumo=asignar_a($r_insumo,NULL,PGSQL_ASSOC)){?>
						<option   value="<?php echo $f_insumo['id_tipo_insumo']?>"> <?php echo "$f_insumo[tipo_insumo]"?></option>
			      <?php
			             }
		              ?>
		              </select></td> 
	...
	<br>
     <tr> 
       <br>
         <td class="tdcampos" title="Generar reporte"><label class="boton" style="cursor:pointer" onclick="Reportmedmayor()" >Procesar</label></td> 
		 <td class="tdcampos" title="Imprimir reporte"><label class="boton" style="cursor:pointer" onclick="ImpReportmedmayor()" >Imprimir</label></td>  
     </tr>              
 </table>
