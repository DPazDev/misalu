...
if ($nomina == 'FECHA CITA') {
    $condicion_nomina = 
        "and gastos_t_b.id_proveedor=proveedores.id_proveedor
		and s_p_proveedores.id_s_p_proveedor=proveedores.id_s_p_proveedor
		and (s_p_proveedores.nomina='1' or s_p_proveedores.nomina='0')";

    $condicion_fecha = 
        "and gastos_t_b.fecha_cita
		between '$fecre1' and '$fecre2'";
} else if ($nomina == 'FECHA ORDEN') {
    $condicion_nomina = 
        "and gastos_t_b.id_proveedor=proveedores.id_proveedor
		and s_p_proveedores.id_s_p_proveedor=proveedores.id_s_p_proveedor
		and (s_p_proveedores.nomina='1' or s_p_proveedores.nomina='0')";
    $condicion_fecha = 
        "and procesos.fecha_recibido
		 between '$fecre1' and '$fecre2'";
}

list($id_sucursal) = explode("@", $_REQUEST['sucur']);
if ($id_sucursal == 0) {
    $condicion_sucursal = "and admin.id_sucursal>0";
} else {
...
if ($estado == 0) {
    $condicion_estado = "and (procesos.id_estado_proceso=7 or procesos.id_estado_proceso=14 or procesos.id_estado_proceso=2)";
} else {
...
if ($id_ente == 0) {
    $condicion_ente = " entes.id_tipo_ente>0";
} else {
...
<table class="tabla_citas" cellpadding=0 cellspacing=0> 
    <tr>
        <td class="titulo_seccion" colspan="19">
            Consultas M&eacute;dicas por Entes, Relaci&oacute;n Orden de Atenci&oacute;n y Emergencia 
        </td>
    </tr>
</table>	
<br>
...
<table class="tabla_citas" cellpadding=0 cellspacing=0> 
    <tr> 
        <td class="tdtitulosd" colspan=19>
            Relaci&oacute;n de <?php echo "$fecre1 al $fecre2"; ?>
        </td>
    </tr> 
    <tr>
        <td colspan=4>&nbsp;</td>
    ...
    <tr> 
        ...
        <td class="tdcampos" colspan=4>
            ...
            if ($estado == 0) 
                ...
                echo "APROBADO OPERADOR"; 
            else if ($estado == 7) 
                echo "CANDIDATO A PAGO"; 
            else if ($estado == 14) 
                echo "ANULADO"; 
            ?> 
            &nbsp; **** PROVEEDORES <?php echo $nomina; ?>
        </td>
    </tr>
    <tr>
        <td>&nbsp;</td>
...
?>
    ...
    $qreporte = 
        "select
            ...
            count(especialidades_medicas.id_especialidad_medica)
        from
            procesos,
            gastos_t_b,
        ...
        where
            ...
            $condicion_fecha 
            and (gastos_t_b.id_servicio='4' or gastos_t_b.id_servicio='6')
            and procesos.id_admin=admin.id_admin  
            $condicion_sucursal 
            $condicion_estado 
            $condicion_nomina
            and procesos.id_titular=titulares.id_titular
            and titulares.id_ente='$fente[id_ente]'
            and estados_procesos.id_estado_proceso=procesos.id_estado_proceso
    ...
    while ($freporte = asignar_a($rreporte, NULL, PGSQL_ASSOC)) {
        ...
        if ($id_ente != '0') {
            echo "
                <td class=\"tdcamposcc\">
                    <a href=\"$url\" title=\"Pacientes por Especialidad\" OnClick=\"Modalbox.show(this.href, {title: this.title, width: 800, height: 400, overlayClose: false}); return false;\" class=\"boton\">
                        $freporte[especialidad_medica]
                    </a>
                </td>
            </tr>
            ";
...
<table class="tabla_citas" colspna=4 cellpadding=0 cellspacing=0> 
    ...
    while ($freporte1 = asignar_a($rreporte1, NULL, PGSQL_ASSOC)) {
        ...
        <?php  
        $contador = $contador + $freporte1['count'];
    }
    ?>
    <tr>
        <td colspan=2 class=\"tdtitulos\">&nbsp;</td>
...
<table>
    ...
    <tr>
        <td colspan=4 class="tdcamposs" title="Imprimir reporte">
            ...
            <?php 
            $url = "'views06/excel_consultas_medicas_x_ente.php?sucur=$id_sucursal&estado=$estado&fecha1=$fecre1&fecha2=$fecre2&nomina=$nomina&ente=$id_ente@$ente&tipo_ente=$tipo_ente@$nom_tipo_ente'";
            ?>
