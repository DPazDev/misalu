...
if($id_servicio==0)	        $condicion_servicio="";
else
$condicion_servicio="and gastos_t_b.id_servicio='$id_servicio'";

list($id_estado)=explode("@",$_REQUEST['edo']);
if($id_estado==0)	        $condicion_estado="and (procesos.id_estado_proceso=2 or procesos.id_estado_proceso=7 or procesos.id_estado_proceso=14 or procesos.id_estado_proceso=17)";
else if($id_estado==-1)	        $condicion_estado="and (procesos.id_estado_proceso=2 or procesos.id_estado_proceso=7)";
...
<?php
$qproceso=("select gastos_t_b.id_proveedor, procesos.id_proceso,procesos.id_estado_proceso,estados_procesos.estado_proceso
            from procesos,estados_procesos,gastos_t_b, admin, $prov where
            gastos_t_b.id_proceso=procesos.id_proceso $condicion_fecha 
            $condicion_servicio and procesos.id_admin=admin.id_admin 
            $condicion_sucursal $condicion_estado  and 
estados_procesos.id_estado_proceso=procesos.id_estado_proceso and
            gastos_t_b.id_proveedor=proveedores.id_proveedor $condicion_proveedor 
            group by gastos_t_b.id_proveedor, procesos.id_proceso,procesos.id_estado_proceso,estados_procesos.estado_proceso
            order by estados_procesos.estado_proceso ");


$rproceso=ejecutar($qproceso);
$HH=0;
	     while($fproceso=asignar_a($rproceso,NULL,PGSQL_ASSOC))
		{
$estadopro="$fproceso[id_estado_proceso]";
...
echo"
<tr>
	        <td class=\"tdtitulos\">  $fproceso[estado_proceso]</td>
	        <td class=\"tdtitulos\">  $fproceso[id_proceso]</td>

</tr> ";}

else if($id_estado==$HH ){
$HH=$id_estado;
...
<tr>
	        <td colspan=4 class="tdcamposs" title="Imprimir reporte">
			  <?php
			$url="'views06/ireporte_consultas_medicas_prov.php?sucur=$id_sucursal&edo=$id_estado&serv=$id_servicio&fecha1=$fecre1&fecha2=$fecre2&proveedor=$proveedor'";
			?> <a href="javascript: imprimir(<?php echo $url; ?>);" class="boton">Imprimir</a>
<?php $url="'views06/excel_consultas_medicas_prov.php?sucur=$id_sucursal&edo=$id_estado&serv=$id_servicio&fecha1=$fecre1&fecha2=$fecre2&proveedor=$proveedor'";
