...
para_entes_tmp_$codigo.no_clave,
para_entes_tmp_$codigo.id_beneficiario,
para_entes_tmp_$codigo.fecha_recibido,
para_entes_tmp_$codigo.nombre,
para_entes_tmp_$codigo.comentarios,
para_entes_tmp_$codigo.comentarios_gerente,
...
<tr><td colspan=23>&nbsp;</td></tr>
	<tr> 
		...
		<td class="descrip_main">FACTURA SENIAT</td>   	   
	   	<td class="descrip_main">SERIE</td>   	   
	   	<td class="descrip_main">FECHA EMISION FACTURA</td>  	 
	   	<td class="descrip_main">ESTADO FACTURA</td>  
	   	<td class="descrip_main">CLAVE</td>
	    <td class="descrip_main">COMENTARIO</td>
	    <td class="descrip_main">COMENTARIO GERENTE OPERACIONES</td>  	     	       	     	     
...
$q_fact_part=("select
				...
				tbl_facturas.id_estado_factura
                            ...
                            where
                                ...
                                tbl_procesos_claves.id_proceso='$freporte1[id_proceso]' and
                                tbl_facturas.id_estado_factura<'3'");
$r_fact_part=ejecutar($q_fact_part);
$f_fact_part=asignar_a($r_fact_part);
...
if($f_fact_part[id_estado_factura]=='1'){
	$estado_fact="PAGADO";
	}
 	else
if($f_fact_part[id_estado_factura]=='2'){
	$estado_fact="POR COBRAR";
	}

$qservicio=("select servicios.servicio from servicios where servicios.id_servicio='$freporte1[id_servicio]' " );
$rservicio=ejecutar($qservicio);
$fservicio=asignar_a($rservicio);


 echo"    <tr>
		<td class=\"tdtituloss\">$freporte1[id_proceso]</td>
		<td class=\"tdtituloss\">$f_fact_part[numero_factura]</td>
		<td class=\"tdtituloss\">$f_fact_part[nomenclatura]</td>
		<td class=\"tdtituloss\">$f_fact_part[fecha_emision]</td>
		<td class=\"tdtituloss\">$estado_fact</td>
		<td class=\"tdtituloss\">$freporte1[no_clave]</td>
	    <td class=\"tdtituloss\">$freporte1[comentarios]</td>
	    <td class=\"tdtituloss\">$freporte1[comentarios_gerente]</td>";
...
echo "       <td class=\"tdtituloss\">$freporte1[n] $freporte1[a]</td>   
		<td class=\"tdtituloss\">$freporte1[cedula]</td>
		<td class=\"tdtituloss\">$freporte1[id_cliente]</td>
		<td class=\"tdtituloss\">$freporte1[fecha_nacimiento]</td>
		<td class=\"tdtituloss\">$sexot</td>
...
if($tipocliente=='TITULAR' or $tipocliente=='BENEFICIARIO' or $tipocliente=='TODOS') {
 echo "
		...
		<td class=\"tdtituloss\">$fservicio[servicio]</td>
";
...
<?php }
		...
		<td colspan=24 class="tdtitulosd" >Total Bs.S.&nbsp;&nbsp;&nbsp;</td>
	        <td colspan=1 class="tdtituloss"> <?php echo montos_print($bsf1); ?></td>
	        <td colspan=1 class="tdtituloss"> <?php echo montos_print($bsf2); ?></td>
<?php }
