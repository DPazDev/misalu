...
else
{
    ...
    $r_factura=pg_query("select tbl_facturas.id_factura, 
        entes.nombre,
    ...
    group by 
        ...
        tbl_series.nomenclatura,
        tbl_facturas.id_serie,
        tbl_facturas.id_estado_factura, tbl_facturas.id_factura                                       
    order by 
        tbl_series.nomenclatura");
...
while($f_factura=pg_fetch_array($r_factura, NULL, PGSQL_ASSOC)) {
    ...
    monto_reserva as noaprobado
    FROM
        gastos_t_b,tbl_procesos_claves
    WHERE
        tbl_procesos_claves.id_factura='$lafacturaes'
        AND gastos_t_b.id_proceso=tbl_procesos_claves.id_proceso
        AND gastos_t_b.id_tipo_servicio=27; ");
...
?>
  	 <tr>
    <td  class="tdcampos" ><?php  echo $f_factura[nomenclatura];?></td>
     <td  class="tdcampos" ><?php echo $f_factura[nombre];?></td>
    <td  class="tdcampos" ><?php  echo montos_print($montofactura);?></td>
	</tr>
