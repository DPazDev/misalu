	...
	if ($tipo_ente==0)
{
  ...
  $TipoEntes="and entes.id_tipo_ente>=$tipo_ente";
	}
	else
	{
		$eltipo_ente="tbl_tipos_entes.id_tipo_ente=$tipo_ente";
...
<table   border=0 class="tabla_citas colortable"  cellpadding=5 >
  <?php
  $r_ente=pg_query("select  * from entes where id_ente=$ente");
  ($f_ente=pg_fetch_array($r_ente, NULL, PGSQL_ASSOC))
  ?>
  <tr>
    <td height="21" colspan="12" class="titulo_seccion">
      <div align="center">
        <strong>Monto de Facturas  Sin Proceso de Auditoria </strong>
      <div>
    </td>
  </tr>

  <tr>
    <th  class="tdcamposc" >Serie</th>
    <th  class="tdcamposc" >Factura</th>
    <th  class="tdcamposc" >Fecha Emision</th>
    <th  class="tdcamposc" >Ente</th>
    <th class="tdcamposc" >Monto</th>
	</tr>
...
while($f_factura=pg_fetch_array($r_factura, NULL, PGSQL_ASSOC))
{
  ...
  $montonoa=("SELECT
    gastos_t_b.id_proceso,monto_reserva as noaprobado
  FROM
    gastos_t_b,tbl_procesos_claves
  WHERE
    tbl_procesos_claves.id_factura='$lafacturaes'
  AND
    gastos_t_b.id_proceso=tbl_procesos_claves.id_proceso
  AND
  ...
  else
  {
    ...
    if ($f_factura[descuento] > 0) {
      $montofactura = $f_factura[sum] - ($f_factura[sum] * ($f_factura[descuento] / 100));
  ...
  <tr>
    ...
    <td  class="tdcamposc" >
      <?php
      if ($forma_pago<>'2'){
        $NumeroFactura=str_pad($f_factura[numero_factura], 7, '0', STR_PAD_LEFT);
        echo "$NumeroFactura";
    ...
    <td  class="tdcamposc" style="text-align:right;"><?php  echo montos_print($montofactura);?></td>
	</tr>
  <?php
}
...
<tr>
  ...
  <td  class="tdcamposc" ></td>
  <td  class="tdcamposc" ></td>
  <th class="tdcamposc" >Total Factura</th>
  <th  class="tdcamposc" ><?php echo  montos_print($totalmontpag1)?></th>
</tr>
</table>
<!---------------NOTAS DE FACTURA(DÉBITO Y CRÉDITO)---->
<?php
      $SqlNotasFactura=("SELECT
                tbl_nota_factura.id_factura,
                tbl_nota_factura.tipo_nota,
                tbl_nota_factura.fecha_emision,
                tbl_nota_factura.monto_nota,
                tbl_nota_factura.num_nota,
                tbl_nota_factura.numcontrolnota,
                tbl_nota_factura.estado_nota,
                tbl_nota_factura.id_serie,
                tbl_nota_factura.id_nota_factura,
                tbl_facturas.numero_factura,
                tbl_series.nomenclatura,
                entes.nombre
              FROM
                public.tbl_nota_factura,
                public.tbl_facturas,
                public.tbl_series,
                entes,
                titulares,
                ...
                tbl_procesos_claves
              WHERE
                tbl_nota_factura.id_factura = tbl_facturas.id_factura AND
                tbl_nota_factura.id_serie = tbl_series.id_serie AND
              ...
              GROUP BY
                ...
                entes.nombre;
                ");
      $RegNotasFactura=ejecutar($SqlNotasFactura);
      if($cuantaNotas=num_filas($RegNotasFactura)>0)
          {
          ?>
          <table border=0 class="tabla_citas colortable"  cellpadding=5>
             <tr>
                <td height="21" colspan="6" class="titulo_seccion">
                  <div align="center">
                    <strong>NOTAS DE CRÉDITO(NC) Y DÉBITO(ND) DEL <?php echo "$fechaini AL $fechafin"; ?></strong>
                  <div>
               </td>
             </tr>
             <tr>
                  <th  class="tdcamposc" >Serie</th>
                  <th  class="tdcamposc" >Tipo Nota</th>
                  <th  class="tdcamposc" >Factura</th>
                  <th  class="tdcamposc" >Fecha Emision</th>
                  <th  class="tdcamposc" >Ente</th>
                  <th class="tdcamposc" >Monto</th>
          ...
          <?php
             ...
             $totalNotaC=0;
             $totalNotaD=0;
           while($NotaFac=pg_fetch_array($RegNotasFactura, NULL, PGSQL_ASSOC))
                {$i++;
                  $SerieNomesclatura=$NotaFac['nomenclatura'];
                  $NumNotaFactura=$NotaFac['num_nota'];
                  $ControlNota=$NotaFac['numcontrolnota'];
                  $NunFactura=$NotaFac['numero_factura'];
                  $id_factura=$NotaFac['id_factura'];
                  $tipoNota=$NotaFac['tipo_nota'];
                  $FechaEmisNota=$NotaFac['fecha_emision'];
                  $montoNota=$NotaFac['monto_nota'];
                  $ente=$NotaFac['nombre'];
                  ////IDENTIFICACION DE LA NOTA
                  if($tipoNota==1)
                    {$totalNotaC=Formato_Numeros($totalNotaC+$montoNota);
                      $NotaTipo='CRÉDITO';
                      $NOTAT='NC. ';
                      $colorCell='color:red;';
                    }
                  ...
                  ////FORMATO NUM FACTURA/NOTA Factura
            			$NumNotaFat=(int) $NumNotaFactura;///Trasformar a entero
                  $NumNotaFat=str_pad($NumNotaFat, 7, '0', STR_PAD_LEFT);
                  ////FORMATO NUM FACTURA
            			$NumFactura=(int) $NunFactura;///Trasformar a entero
                  $NumFactura=str_pad($NunFactura, 7, '0', STR_PAD_LEFT);


                ?>

                 <tr>
                      <td  class="tdcamposc" ><?php echo  $SerieNomesclatura;?></td>
                      <td  class="tdcamposc" ><?php echo  $NotaTipo;?></td>
                      <td  class="tdcamposc" ><?php echo  "$NOTAT $NumNotaFat FAC ($NumFactura)";?></td>
                      <td  class="tdcamposc" ><?php echo  $FechaEmisNota;?></td>
                      <td  class="tdcamposc" ><?php echo  $ente;?></td>
                      <td  class="tdcamposc" style="text-align:right;"><?php echo "<span style='$colorCell'> $montoNota </span>";?></td>
                 </tr>
              <?php
                }
              ...
              ?>
            <tr>
              <td  class="tdcamposc" colspan="4"></td>
              <th  class="tdcamposc" style="text-align:right;">Total Débito</th>
              <th  class="tdcamposc" style="text-align:right;">(+)<?php echo  montos_print($totalNotaD)?></th>
            </tr>
            <tr>
              <td  class="tdcamposc" colspan="4"></td>
              <th  class="tdcamposc" style="text-align:right;">Total Crédito</th>
              <th  class="tdcamposc" style="text-align:right;">(-)<?php echo  montos_print($totalNotaC)?></th>
            </tr>
          </table>
<?php
      }
//////////////FIN NOTAS DE FACTURA
?>
