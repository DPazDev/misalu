...
$objPHPExcel->getProperties()->setCreator("$f_admin[nombres] $f_admin[apellidos]")
							 ...
							 ->setDescription("Reporte que muestra Relacion Venta de Polizas Individuales")
							 ->setKeywords("office 2007 openxml php")
							 ->setCategory("Test result file");


$objPHPExcel->getActiveSheet(0)->mergeCells('A3:E3');
$objPHPExcel->getActiveSheet()->setCellValue('A3', "Reporte Relación Venta de Pólizas Individuales por Fecha de Vigencia de Contrato desde el $fecre1 al $fecre2 ");
...
$objPHPExcel->getActiveSheet(0)->setCellValue("A".$i, '')
                    ...
                    ->setCellValue("J".$i, 'FECHA ULTIMO PAGO')
                    ->setCellValue("K".$i, 'SUBDIVISION')
                    ->setCellValue("L".$i, 'FECHA INICIO')
                    ->setCellValue("M".$i, 'FECHA FIN')
                    ->setCellValue("N".$i, 'COMISIONADO')
                    ->setCellValue("O".$i, 'RECIBO PRIMA')
                    ->setCellValue("P".$i, 'MONTO CONTRATO')	
                    ->setCellValue("Q".$i, 'MONTO CANCELADO')
                    ->setCellValue("R".$i, 'MONTO DEUDOR')
                    ->setCellValue("S".$i, 'FECHA PROXIMA DE PAGO')
		    ->setCellValue("T".$i, 'PLAN')
                    ->setCellValue("U".$i, 'ES')
...
$q_venta=("select 
	...
	comisionados.nombres, comisionados.apellidos, 
	sum(tbl_caract_recibo_prima.monto_prima)
	from entes, clientes,titulares, tbl_contratos_entes, tbl_recibo_contrato, comisionados, tbl_caract_recibo_prima, primas,  subdivisiones, titulares_subdivisiones 
	where 
	tbl_recibo_contrato.fecha_ini_vigencia between '$fecre1' and '$fecre2'  and 
	entes.id_tipo_ente=7 and 
	tbl_caract_recibo_prima.id_titular=titulares.id_titular and
...
$objPHPExcel->getActiveSheet(0)->setCellValueExplicit("J".$i, $f_cuota[fecha_pago], PHPExcel_Cell_DataType::TYPE_STRING);
$objPHPExcel->getActiveSheet(0)->setCellValueExplicit("K".$i, $f_venta[subdivision], PHPExcel_Cell_DataType::TYPE_STRING);
$objPHPExcel->getActiveSheet(0)->setCellValueExplicit("L".$i, $f_venta[fecha_ini_vigencia], PHPExcel_Cell_DataType::TYPE_STRING);
$objPHPExcel->getActiveSheet(0)->setCellValueExplicit("M".$i, $f_venta[fecha_fin_vigencia], PHPExcel_Cell_DataType::TYPE_STRING);
$objPHPExcel->getActiveSheet(0)->setCellValueExplicit("N".$i, "$f_venta[nombres] $f_venta[apellidos]", PHPExcel_Cell_DataType::TYPE_STRING);
$objPHPExcel->getActiveSheet(0)->setCellValueExplicit("O".$i, $f_venta[num_recibo_prima], PHPExcel_Cell_DataType::TYPE_STRING);
$objPHPExcel->getActiveSheet(0)->setCellValue("P".$i, $f_venta[sum]);
$objPHPExcel->getActiveSheet(0)->setCellValue("Q".$i, $f_montopagado[sum]);
$objPHPExcel->getActiveSheet(0)->setCellValue("R".$i, $c);
$objPHPExcel->getActiveSheet(0)->setCellValue("S".$i, $cuota);
$objPHPExcel->getActiveSheet(0)->setCellValueExplicit("T".$i, $planes, PHPExcel_Cell_DataType::TYPE_STRING);
$objPHPExcel->getActiveSheet(0)->setCellValue("U".$i, $varrenovo);
$objPHPExcel->getActiveSheet(0)->setCellValue("V".$i, $edo, PHPExcel_Cell_DataType::TYPE_STRING);
...
->setFormatCode('#,##0.00');
$objPHPExcel->getActiveSheet()->getStyle("R6:R$i")->getNumberFormat()
->setFormatCode('#,##0.00');
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('Ventas_Individuales');
...
header("Content-Disposition: attachment; filename=relacion_Venta_Polizas_Individuales_$numealeatorio.xls");
