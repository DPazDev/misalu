...
function validarcorreo(correo,ms=0){
    var regex = /^[-\w.%+]{1,64}@(?:[A-Z0-9-]{1,63}\.){1,125}[A-Z]{2,63}$/i;
  if(!regex.test(correo)){
		if(ms>0){
      alert('Email es invalido, Verificar xxx@xxx.com')
    }
		return false;
	}else{
    return true;
...
function limpiarSoloNumeros(tfn){
  //ayuda a dejar solo numero del telefono
  tef=tfn.replace(/\s+/g, '') //elimiar espacios en blanco
  tef=tef.replace(/[A-Za-z]|[\(\).-]+/g,"");//eliminar caracteres esecilaes y letras
  return tef;
}

function formatoTelefono(e){
  ///dar formato a un telefono usar onkeyup="return formatoTelefono(event);" en el campo
    var key = window.Event ? e.which : e.keyCode;
    let Tecla=e.key;
    let value = e.target.value;
    let id = e.target.id;
    tef=limpiarSoloNumeros(value);//borrar todos los caracteres y letras
    nun=tef.length;//cuantos digitos
    myRe =/[0-9]+/g;//solo numeros
    borraracter=0;
    if(myRe.test(Tecla)){
      if(nun>11){//borrar si sobrepasa los 11
        borraracter=1;
      }
    }

    if(borraracter==1){
      ntef=tef.substring(0, tef.length - 1)
      e.target.value =ntef;
      tef=ntef;
    }
    //Formato de telefono
    e.target.value = tef.replace(/\W/gi, '').replace(/(.{4})(.{3})(.{4})/g, '($1)-$2-$3').replace(/\s$/,'');
}
/*
...
//formato 2:DD-MM-AAAA formto 1:AAAA-MM-DD
...
function ModificarTazaMoneda(idmoneda,act){
      //Actualiza el la taza de cambio de la moneda
...
function facturasfaltantesLibro(){
  idProvedor=$F('proveedor');
  FechaIni=$F('fechaini');
  FechaFin=$F('fechafin');
 if(idProvedor=='' || FechaIni=='' || FechaFin==''){
   alert('debes selecionar un rango de fecha y algun proveedor o todos');
 }else{
    new Ajax.Request("views05/trasladar_facturas_sinRetencion.php",
        {
          method:'post',
          asynchronous: true,
          postBody: 'idProvedor='+idProvedor+'&FechaIni='+FechaIni+'&FechaFin='+FechaFin,
          onCreate: function(response) {
            a= Ajax.activeRequestCount;
            if(a>=0)
              $('spinner' ).show();
...
///permite selecionar todos los checkbox y activarlos pasando el contenedor por defecto Form
 function checkAll(idformulario='Form') {
          document.querySelectorAll('#'+idformulario+' input[type=checkbox]').forEach(function(checkElement) {
           checkElement.checked = true;
       });
   }
   ///permite deselecionar todos los checkbox pasandole el contenedor por defecto Form
 function uncheckAll(idformulario='Form') {
       document.querySelectorAll('#'+idformulario+' input[type=checkbox]').forEach(function(checkElement) {
           checkElement.checked = false;
       });
   }

function facturasfaltantesLibro2()
{		  honorarios1="";
			factura1="";
			idordcom1="";
			montoexento1="";
      ...
      j=0;
        ...
        idproveedor+=spd+document.getElementById("idproveedor_"+[j]).value;
        nombreprov+=spd+document.getElementById("nombreprov_"+[j]).value;
				honorarios1 +=spd+honorarios.value;
				factura1 +=spd+factura.value;
				idordcom1 +=spd+idordcom.value;
				montoexento1 +=spd+montoexento.value;
				baseimponible1 +=spd+baseimponible.value;
				iva_fact1 +=spd+iva_fact.value;
				iva_ret1 +=spd+iva_ret.value;
				confactura1 +=spd+confactura.value;
        fecha_emision1 +=spd+fecha_emision.value;


				}
			}

//recibir parametros necesario
      conexa=$F('conexa');
      banco=$F('banco');
      numcheque=$F('numcheque');
      direccionprov=$F('direccionprov');
      rif=$F('rif');
      nombreprov=$F('nombreprov');
      anombrede=$F('anombrede');
      cedularif=$F('cedularif');
      motivo=$F('motivo');
      ...
      new Ajax.Request("views05/trasladar_facturas_sinRetencion2.php",
          {
             method:'post',
             asynchronous: true,
             postBody:
      	      '&conexa='+conexa+'&honorarios1='+honorarios1+
              '&banco='+banco+
              '&numcheque='+numcheque+
              '&factura1='+factura1+
              '&idordcom1='+idordcom1+
              '&rif='+rif+
              '&montoexento1='+montoexento1+
              '&baseimponible1='+baseimponible1+
              '&iva_fact1='+iva_fact1+
              '&iva_ret1='+iva_ret1+
              '&nombreprov='+nombreprov+
              '&confactura1='+confactura1+
      ...
      function MostrarRetFActuras(req)
      {
...
////////////////////////////////////
function sumarfacCompras(elem){

	//if (document.u.p.value.length == 0 || document.u.l.value.length == 0){
		var oa = document.getElementById("FormFacturasCompras");
			var j=0;
			var montot=0;
            var ivarett=0;
...
document.forms['FormFacturasCompras']['monto'].value=montot.toFixed(2);
document.forms['FormFacturasCompras']['iva_rettt'].value=ivarett.toFixed(2);
}
...
function buscarfacturadeb(){
		if ((Field.present('factura')==false))
		{
			...
			{
	new Ajax.Request("views04/reg_notadebito2.php",
	    {
	       method:'post',
	       asynchronous: true,
	       postBody:'&factura='+$F('factura'),
				 onComplete: mos_bus_fact_deb

	    });
		var1 = document.getElementById('buscarfacturadeb');
			var1.innerHTML = '<img src="../public/images/esperar.gif"><br><span  class="titulos">Cargando...</span>';
	}
		function mos_bus_fact_deb(req){

		  $('buscarfacturadeb').innerHTML= req.responseText;

		}
}

function CambioNotaDebito(monto,cambio,idcampo){
/* franklin monsalve funcion para calcular un monto segun
el cambio de referencia (cambio)e insertar el monto en un campo usando referencia el id del campo(idcampo) de referencia */
  valorCambio=monto*cambio;
  valorCambio=formatoNumeroDec(valorCambio);
  $(idcampo).value=valorCambio;
}
///Guardar Nota de debito
function guardar_notadebito()
{//guardar datos de nota de debito para anexar(+) nuevo cobro
  ///verificar Campos

  id_factura=$F('id_factura');
  id_serie=$F('serie');
  ControlNotaD=$F('controlfactura').trim();//control del numero nota de debito
  UltimoControlFactura=$F('UltimoControlFactura').trim();//control del numero factura de la serie
  NumNotaDebito=$F('num_notadebito').trim();
  FechaNotaD=$F('dateField3').trim();
  concepto=$F('concepto').trim();
  montoDebito=$F('monto');//monto en bs o dolares
  montoTotalDebito=$F('MontoTotal');//trasformacion a bs y total a pagar
  TipoNotadebito=$F('TipoNotadebito').trim();
...
///validar datos
  ...
  if (montoDebito<=0 || isNaN(montoDebito))
    ...
    if(Validarcrt==false || Validarc==false || Validarf==false || Validarm==false){
            alert("Error:"+msj1+"");
            Validar=false;
        }else{Validar=true;}

if(Validar==true){
		new Ajax.Request("views04/reg_notadebito3.php",
		{
		method:'post',
		asynchronous: true,
		postBody:
		'id_factura='+id_factura+'&dateField3='+FechaNotaD+'&concepto='+concepto+'&controlfactura='+ControlNotaD+'&num_notadebito='+NumNotaDebito+'&monto='+montoTotalDebito+'&tipo_debito='+TipoNotadebito+'&idserie='+id_serie,
	onComplete: mos_gua_nota_debito
	...
	function mos_gua_nota_debito(req)
	{
	   $('buscarfacturadeb').innerHTML= req.responseText;
	}
}

/////Funcion modificar Datos de te de contacto Clientes
function mod_datos_contacto(idcli=0,dv='data-cliente'){
    // ci cedula Cliente y dv ///donde se mostrara los datos actualizados
    msc='';
    error=0;
    email=$F('email');
    tlf1=$F('telef1');
  ...
  ///limpiar telefonos solo numero
  tfn1=limpiarSoloNumeros(tlf1);
  tfn2=limpiarSoloNumeros(tlf2);
  tfn3=limpiarSoloNumeros(tlf3);
  ///verificar solo 11 digitos
  errort=0;
  if(tfn1.length<11)
    {errort++; }
  if(tfn2.length>0)
...
////verificar errores
  if(errort>0)
  {msc+='Numero de telefono invalido1\n';
  error++; }
	contenedor = document.getElementById(dv);
	...
	if(error>0){
		alert(msc);
      	...
      	ajax=nuevoAjax();
      	ajax.open("GET", "views01/mod_datos_contacto2.php?idcli="+idcli+"&correo="+email+"&telef="+tlf1+"&celular1="+tlf2+"&celular2="+tlf3,true);
      	ajax.onreadystatechange=function() {
      	if (ajax.readyState==4) {
      		contenedor.innerHTML = ajax.responseText
      		}
